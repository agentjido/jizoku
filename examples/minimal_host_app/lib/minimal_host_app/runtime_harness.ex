defmodule MinimalHostApp.RuntimeHarness do
  ...
  Jizoku boundary, so runtime setup and durable run polling live here
  ...
  def wait_for_execution do
    if manual_oban_testing?() do
      _result = Oban.drain_queue(queue: :jizoku, with_recursion: true)
      :ok
    else
      :ok
    end
  ...
  def drain_available_jobs(limit \\ 1) when is_integer(limit) and limit > 0 do
    if manual_oban_testing?() do
      Oban.drain_queue(queue: :jizoku, with_limit: limit)
    else
      :ok
    end
  end
  ...
  defp library_migrations_path do
    Application.app_dir(:jizoku, "priv/repo/migrations")
  end
