defmodule BedrockMinimalHostApp.SquidieLeaseAdapterTest do
  use ExUnit.Case, async: false

  alias Ecto.Adapters.SQL.Sandbox
  alias BedrockMinimalHostApp.Jobs.SquidiePayload
  alias BedrockMinimalHostApp.JobQueue
  alias BedrockMinimalHostApp.Repo
  alias BedrockMinimalHostApp.RuntimeSignals
  alias BedrockMinimalHostApp.SquidieLeaseAdapter
  ...
  setup do
    ...
    original_payload_config = Application.get_env(:bedrock_minimal_host_app, SquidiePayload, [])

    on_exit(fn ->
      Application.put_env(:bedrock_minimal_host_app, SquidiePayload, original_payload_config)
    end)
  ...
  test "claims, heartbeats, and completes leased Squidie payloads", %{queue: queue} do
    ...
    assert {:ok, [claim]} =
             SquidieLeaseAdapter.claim(%{}, queue, "worker_a",
    ...
    assert {:ok, []} =
             SquidieLeaseAdapter.claim(%{}, queue, "worker_b",
               lease_duration_ms: 1_000,
               now: now + 500
             )

    assert {:ok, heartbeated_claim} =
             SquidieLeaseAdapter.heartbeat(%{}, claim,
    ...
    assert :ok = SquidieLeaseAdapter.complete(%{}, heartbeated_claim, [])
    assert {:ok, []} = SquidieLeaseAdapter.claim(%{}, queue, "worker_c", now: now + 5_500)
  ...
  test "fails claims through backend retry and dead-letter policy", %{queue: queue} do
    ...
    assert {:ok, [claim]} = SquidieLeaseAdapter.claim(%{}, queue, "worker_a", now: now)

    assert {:ok, :requeued} =
             SquidieLeaseAdapter.fail(%{}, claim, %{message: "gateway timeout"},
               base_delay: 1_000,
               now: now
             )

    assert {:ok, []} = SquidieLeaseAdapter.claim(%{}, queue, "worker_b", now: now + 999)

    assert {:ok, [retry_claim]} =
             SquidieLeaseAdapter.claim(%{}, queue, "worker_b", now: now + 1_000)
    ...
    assert {:ok, :dead_lettered} =
             SquidieLeaseAdapter.fail(%{}, retry_claim, %{message: "still failing"},
               base_delay: 1_000,
               now: now + 1_000
             )
  ...
  test "executes a journal workflow while holding a Bedrock lease", %{queue: queue} do
    ...
    assert {:ok, _item_id} = JobQueue.enqueue(queue, "squidie:payload", payload)
    assert {:ok, [claim]} = SquidieLeaseAdapter.claim(%{}, queue, "worker_a", [])

    assert :ok = SquidiePayload.perform(claim.payload, %{})
    assert :ok = SquidieLeaseAdapter.complete(%{}, claim, [])
  ...
  test "applies cancellation through the example app signal boundary" do
    ...
    with_squidie_queue(queue, fn ->
      ...
      assert {:ok, signal} =
               Squidie.Runtime.Signal.cancel_run(started_run.run_id,
  ...
  test "applies manual control signals through the example app boundary" do
    ...
    with_squidie_queue(queue, fn ->
      ...
      assert {:ok, %Squidie.ReadModel.Inspection.Snapshot{status: :paused} = paused_run} =
               Squidie.execute_next(owner_id: "bedrock-manual-approve-test")
      ...
      assert {:ok, %Squidie.ReadModel.Inspection.Snapshot{status: :completed}} =
               Squidie.execute_next(owner_id: "bedrock-manual-approve-complete-test")

      assert {:ok, rejection_run} =
               WorkflowRuns.start_manual_approval(%{account_id: "acct_bedrock_reject"})

      assert {:ok, %Squidie.ReadModel.Inspection.Snapshot{status: :paused}} =
               Squidie.execute_next(owner_id: "bedrock-manual-reject-test")
      ...
      assert {:ok, %Squidie.ReadModel.Inspection.Snapshot{status: :completed}} =
               Squidie.execute_next(owner_id: "bedrock-manual-reject-complete-test")

      assert {:ok, pause_run} =
               WorkflowRuns.start_manual_pause(%{account_id: "acct_bedrock_resume"})

      assert {:ok, %Squidie.ReadModel.Inspection.Snapshot{status: :paused}} =
               Squidie.execute_next(owner_id: "bedrock-manual-resume-test")
  ...
  test "executes payment recovery through a Bedrock lease with runtime attempt metadata",
    ...
    with_squidie_queue(queue, fn ->
      Application.put_env(:bedrock_minimal_host_app, SquidiePayload,
      ...
      assert {:ok, _item_id} = JobQueue.enqueue(queue, "squidie:payload", drain_payload(queue))
      assert {:ok, [claim]} = SquidieLeaseAdapter.claim(%{}, queue, "worker_payment", [])
      ...
      assert {:ok, completed_graph} = Squidie.inspect_run_graph(started_run.run_id)

      graph_nodes =
        Map.new(Squidie.Runs.GraphInspection.to_map(completed_graph).nodes, &{&1.id, &1})
  ...
  test "keeps Bedrock payment recovery retry errors inspectable with attempt metadata",
    ...
    with_squidie_queue(queue, fn ->
      ...
      Application.put_env(:bedrock_minimal_host_app, SquidiePayload, max_journal_attempts: 2)

      assert {:ok, _item_id} = JobQueue.enqueue(queue, "squidie:payload", drain_payload(queue))
      assert {:ok, [claim]} = SquidieLeaseAdapter.claim(%{}, queue, "worker_payment_retry", [])

      assert {:error, :journal_drain_limit_exceeded} =
               SquidiePayload.perform(claim.payload, %{})
  ...
  test "executes a nested journal workflow through a Bedrock lease and drains child retry separately",
    ...
    Application.put_env(:bedrock_minimal_host_app, SquidiePayload, max_journal_attempts: 1)

    assert {:ok, _item_id} = JobQueue.enqueue(queue, "squidie:payload", payload, now: now)
    assert {:ok, [claim]} = SquidieLeaseAdapter.claim(%{}, queue, "worker_a", now: now)

    assert {:error, :journal_drain_limit_exceeded} = SquidiePayload.perform(claim.payload, %{})

    assert {:ok, [parent_summary]} =
             Squidie.list_runs(workflow: BedrockMinimalHostApp.Workflows.NestedInviteDelivery)
    ...
    assert {:ok, parent_graph} = Squidie.inspect_run_graph(parent_summary.run_id)
    ...
    assert {:ok, reconstructed_parent_graph} = Squidie.inspect_run_graph(retried_parent.run_id)
    ...
    assert {:ok, :requeued} =
             SquidieLeaseAdapter.fail(%{}, claim, %{message: "parent drain limit"},
               base_delay: 1,
               now: now
             )

    assert {:ok, [parent_retry_claim]} =
             SquidieLeaseAdapter.claim(%{}, queue, "worker_b", now: now + 1)

    Application.put_env(:bedrock_minimal_host_app, SquidiePayload, max_journal_attempts: 50)

    assert :ok = SquidiePayload.perform(parent_retry_claim.payload, %{})
    assert :ok = SquidieLeaseAdapter.complete(%{}, parent_retry_claim, [])
    ...
    assert {:ok, _item_id} =
             JobQueue.enqueue(queue, "squidie:payload", drain_payload, now: child_drain_now)

    assert {:ok, [child_drain_claim]} =
             SquidieLeaseAdapter.claim(%{}, queue, "worker_child_a", now: child_drain_now)

    assert {:error, :journal_drain_limit_exceeded} =
             SquidiePayload.perform(child_drain_claim.payload, %{})
    ...
    assert {:ok, :requeued} =
             SquidieLeaseAdapter.fail(%{}, child_drain_claim, %{message: "child drain limit"},
               base_delay: 1,
               now: child_drain_now
             )

    assert {:ok, [child_retry_claim]} =
             SquidieLeaseAdapter.claim(%{}, queue, "worker_child_b", now: child_drain_now + 1)

    Application.put_env(:bedrock_minimal_host_app, SquidiePayload, max_journal_attempts: 50)

    assert :ok = SquidiePayload.perform(child_retry_claim.payload, %{})
    assert :ok = SquidieLeaseAdapter.complete(%{}, child_retry_claim, [])
  ...
  test "requeues a leased journal payload after a bounded drain failure", %{queue: queue} do
    ...
    assert {:ok, :requeued} =
             SquidieLeaseAdapter.fail(%{}, claim, %{message: "drain limit"},
               base_delay: 1,
               now: now
             )
    ...
    assert :ok = SquidiePayload.perform(retry_claim.payload, %{})
    assert :ok = SquidieLeaseAdapter.complete(%{}, retry_claim, [])
  ...
  defp with_squidie_queue(queue, fun) when is_function(fun, 0) do
    ...
    after
      if is_nil(original_queue) do
        Application.delete_env(:squidie, :queue)
      else
        Application.put_env(:squidie, :queue, original_queue)
      end
    end
  end
end
