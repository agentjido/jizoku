defmodule BedrockMinimalHostApp.JizokuLeaseAdapterTest do
  use ExUnit.Case, async: false

  alias Ecto.Adapters.SQL.Sandbox
  alias BedrockMinimalHostApp.Jobs.JizokuPayload
  alias BedrockMinimalHostApp.JobQueue
  alias BedrockMinimalHostApp.Repo
  alias BedrockMinimalHostApp.RuntimeSignals
  alias BedrockMinimalHostApp.JizokuLeaseAdapter
  ...
  setup do
    ...
    original_payload_config = Application.get_env(:bedrock_minimal_host_app, JizokuPayload, [])

    on_exit(fn ->
      Application.put_env(:bedrock_minimal_host_app, JizokuPayload, original_payload_config)
    end)
  ...
  test "claims, heartbeats, and completes leased Jizoku payloads", %{queue: queue} do
    ...
    assert {:ok, [claim]} =
             JizokuLeaseAdapter.claim(%{}, queue, "worker_a",
    ...
    assert {:ok, []} =
             JizokuLeaseAdapter.claim(%{}, queue, "worker_b",
               lease_duration_ms: 1_000,
               now: now + 500
             )

    assert {:ok, heartbeated_claim} =
             JizokuLeaseAdapter.heartbeat(%{}, claim,
    ...
    assert :ok = JizokuLeaseAdapter.complete(%{}, heartbeated_claim, [])
    assert {:ok, []} = JizokuLeaseAdapter.claim(%{}, queue, "worker_c", now: now + 5_500)
  ...
  test "fails claims through backend retry and dead-letter policy", %{queue: queue} do
    ...
    assert {:ok, [claim]} = JizokuLeaseAdapter.claim(%{}, queue, "worker_a", now: now)

    assert {:ok, :requeued} =
             JizokuLeaseAdapter.fail(%{}, claim, %{message: "gateway timeout"},
               base_delay: 1_000,
               now: now
             )

    assert {:ok, []} = JizokuLeaseAdapter.claim(%{}, queue, "worker_b", now: now + 999)

    assert {:ok, [retry_claim]} =
             JizokuLeaseAdapter.claim(%{}, queue, "worker_b", now: now + 1_000)
    ...
    assert {:ok, :dead_lettered} =
             JizokuLeaseAdapter.fail(%{}, retry_claim, %{message: "still failing"},
               base_delay: 1_000,
               now: now + 1_000
             )
  ...
  test "executes a journal workflow while holding a Bedrock lease", %{queue: queue} do
    ...
    assert {:ok, _item_id} = JobQueue.enqueue(queue, "jizoku:payload", payload)
    assert {:ok, [claim]} = JizokuLeaseAdapter.claim(%{}, queue, "worker_a", [])
  ...
  test "applies cancellation through the example app signal boundary" do
    ...
    with_jizoku_queue(queue, fn ->
      ...
      assert {:ok, signal} =
               Jizoku.Runtime.Signal.cancel_run(started_run.run_id,
  ...
  test "applies manual control signals through the example app boundary" do
    ...
    with_jizoku_queue(queue, fn ->
      ...
      assert {:ok, %Jizoku.ReadModel.Inspection.Snapshot{status: :paused} = paused_run} =
               Jizoku.execute_next(owner_id: "bedrock-manual-approve-test")
      ...
      assert {:ok, %Jizoku.ReadModel.Inspection.Snapshot{status: :completed}} =
               Jizoku.execute_next(owner_id: "bedrock-manual-approve-complete-test")

      assert {:ok, rejection_run} =
               WorkflowRuns.start_manual_approval(%{account_id: "acct_bedrock_reject"})

      assert {:ok, %Jizoku.ReadModel.Inspection.Snapshot{status: :paused}} =
               Jizoku.execute_next(owner_id: "bedrock-manual-reject-test")
      ...
      assert {:ok, %Jizoku.ReadModel.Inspection.Snapshot{status: :completed}} =
               Jizoku.execute_next(owner_id: "bedrock-manual-reject-complete-test")

      assert {:ok, pause_run} =
               WorkflowRuns.start_manual_pause(%{account_id: "acct_bedrock_resume"})

      assert {:ok, %Jizoku.ReadModel.Inspection.Snapshot{status: :paused}} =
               Jizoku.execute_next(owner_id: "bedrock-manual-resume-test")
  ...
  test "executes payment recovery through a Bedrock lease with runtime attempt metadata",
    ...
    with_jizoku_queue(queue, fn ->
      Application.put_env(:bedrock_minimal_host_app, JizokuPayload,
      ...
      assert {:ok, _item_id} = JobQueue.enqueue(queue, "jizoku:payload", drain_payload(queue))
      assert {:ok, [claim]} = JizokuLeaseAdapter.claim(%{}, queue, "worker_payment", [])
      ...
      assert {:ok, completed_graph} = Jizoku.inspect_run_graph(started_run.run_id)

      graph_nodes =
        Map.new(Jizoku.Runs.GraphInspection.to_map(completed_graph).nodes, &{&1.id, &1})
  ...
  test "keeps Bedrock payment recovery retry errors inspectable with attempt metadata",
    ...
    with_jizoku_queue(queue, fn ->
      ...
      Application.put_env(:bedrock_minimal_host_app, JizokuPayload, max_journal_attempts: 2)

      assert {:ok, _item_id} = JobQueue.enqueue(queue, "jizoku:payload", drain_payload(queue))
      assert {:ok, [claim]} = JizokuLeaseAdapter.claim(%{}, queue, "worker_payment_retry", [])

      assert {:error, :journal_drain_limit_exceeded} =
               JizokuPayload.perform(claim.payload, %{})
  ...
  test "executes a nested journal workflow through a Bedrock lease and drains child retry separately",
    ...
    Application.put_env(:bedrock_minimal_host_app, JizokuPayload, max_journal_attempts: 1)

    assert {:ok, _item_id} = JobQueue.enqueue(queue, "jizoku:payload", payload, now: now)
    assert {:ok, [claim]} = JizokuLeaseAdapter.claim(%{}, queue, "worker_a", now: now)

    assert {:error, :journal_drain_limit_exceeded} = JizokuPayload.perform(claim.payload, %{})

    assert {:ok, [parent_summary]} =
             Jizoku.list_runs(workflow: BedrockMinimalHostApp.Workflows.NestedInviteDelivery)
    ...
    assert {:ok, parent_graph} = Jizoku.inspect_run_graph(parent_summary.run_id)
    ...
    assert {:ok, reconstructed_parent_graph} = Jizoku.inspect_run_graph(retried_parent.run_id)
    ...
    assert {:ok, :requeued} =
             JizokuLeaseAdapter.fail(%{}, claim, %{message: "parent drain limit"},
               base_delay: 1,
               now: now
             )

    assert {:ok, [parent_retry_claim]} =
             JizokuLeaseAdapter.claim(%{}, queue, "worker_b", now: now + 1)

    Application.put_env(:bedrock_minimal_host_app, JizokuPayload, max_journal_attempts: 50)

    assert :ok = JizokuPayload.perform(parent_retry_claim.payload, %{})
    assert :ok = JizokuLeaseAdapter.complete(%{}, parent_retry_claim, [])
    ...
    assert {:ok, _item_id} =
             JobQueue.enqueue(queue, "jizoku:payload", drain_payload, now: child_drain_now)

    assert {:ok, [child_drain_claim]} =
             JizokuLeaseAdapter.claim(%{}, queue, "worker_child_a", now: child_drain_now)

    assert {:error, :journal_drain_limit_exceeded} =
             JizokuPayload.perform(child_drain_claim.payload, %{})
    ...
    assert {:ok, :requeued} =
             JizokuLeaseAdapter.fail(%{}, child_drain_claim, %{message: "child drain limit"},
               base_delay: 1,
               now: child_drain_now
             )

    assert {:ok, [child_retry_claim]} =
             JizokuLeaseAdapter.claim(%{}, queue, "worker_child_b", now: child_drain_now + 1)

    Application.put_env(:bedrock_minimal_host_app, JizokuPayload, max_journal_attempts: 50)

    assert :ok = JizokuPayload.perform(child_retry_claim.payload, %{})
    assert :ok = JizokuLeaseAdapter.complete(%{}, child_retry_claim, [])
  ...
  test "requeues a leased journal payload after a bounded drain failure", %{queue: queue} do
    ...
    assert {:ok, :requeued} =
             JizokuLeaseAdapter.fail(%{}, claim, %{message: "drain limit"},
               base_delay: 1,
               now: now
             )
    ...
    assert :ok = JizokuPayload.perform(retry_claim.payload, %{})
    assert :ok = JizokuLeaseAdapter.complete(%{}, retry_claim, [])
  ...
  defp with_jizoku_queue(queue, fun) when is_function(fun, 0) do
    ...
    after
      if is_nil(original_queue) do
        Application.delete_env(:jizoku, :queue)
      else
        Application.put_env(:jizoku, :queue, original_queue)
      end
    end
  end
end
