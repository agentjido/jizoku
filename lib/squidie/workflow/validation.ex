defmodule Squidie.Workflow.Validation do
  @moduledoc """
  ...
  `Squidie.Workflow` can remain compact and declarative.
  """
  ...
  defp validate_transition_condition(errors, %{condition: condition, from: from}) do
    case Squidie.Workflow.TransitionCondition.normalize(condition) do
      {:ok, _condition} ->
        errors
  ...
  defp duplicate_transition_condition?(transitions) do
    conditions =
      ...
      |> Enum.map(&Squidie.Workflow.TransitionCondition.serialize(&1.condition))
