defmodule Squidie.Workflow.Definition do
  ...
  `Squidie.Workflow` builds the declarative DSL at compile time. This module
  ...
  @type transition_condition :: Squidie.Workflow.TransitionCondition.t()
  ...
  transaction. Squidie persists attempt, step, and run progression in its
  ...
  defp transition_matches?(%{condition: condition}, context) do
    Squidie.Workflow.TransitionCondition.matches?(context, condition)
  end
  ...
  defp maybe_put_serialized_condition(serialized, condition) do
    case Squidie.Workflow.TransitionCondition.serialize(condition) do
      nil -> serialized
  ...
  defp maybe_put_deserialized_condition(transition, condition) do
    case Squidie.Workflow.TransitionCondition.deserialize(condition) do
      nil -> transition
