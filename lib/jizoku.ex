...
defmodule Jizoku do
  @moduledoc """
  Public entrypoint for the Jizoku runtime.
  ...
  alias Jizoku.Config
  alias Jizoku.ReadModel.ContinuationChain
  ...
  alias Jizoku.Runtime.Journal.GraphMutation
  ...
  alias Jizoku.Runtime.Signal.JidoIngress
  ...
  @doc """
  Loads Jizoku configuration from the application environment with optional
  runtime overrides.
  """
  ...
  @doc """
  Loads Jizoku configuration and raises if required keys are missing.
  """
  ...
  Public manual starts load journal runtime defaults from `config :jizoku`.
  When `journal_storage:` is not passed explicitly, Jizoku infers Ecto-backed
  journal storage from the configured `:repo`, so host apps must either set
  `config :jizoku, repo: MyApp.Repo` globally or pass an explicit
  ...
  `config :jizoku`, so host apps must keep `:repo` configured globally unless
  ...
  before activation. Passing `:action_registry` lets Jizoku resolve stable
  action keys to approved executable modules at the start boundary.
  """
  @spec start_spec(Jizoku.Workflow.Spec.t() | map(), map(), keyword()) ::
          {:ok, Jizoku.ReadModel.Inspection.Snapshot.t()}
          | {:error, Config.config_error()}
          | {:error, start_option_error()}
          | {:error, Starter.start_error()}
  ...
  """
  @spec start_spec(Jizoku.Workflow.Spec.t() | map(), atom(), map(), keyword()) ::
          {:ok, Jizoku.ReadModel.Inspection.Snapshot.t()}
          | {:error, Config.config_error()}
          | {:error, start_option_error()}
          | {:error, Starter.start_error()}
  ...
  """
  @spec preview_spec(Jizoku.Workflow.Spec.t() | map(), map(), keyword()) ::
          {:ok, Jizoku.Runs.SpecPreview.t()} | {:error, term()}
  ...
  """
  @spec preview_spec(Jizoku.Workflow.Spec.t() | map(), atom(), map(), keyword()) ::
          {:ok, Jizoku.Runs.SpecPreview.t()} | {:error, term()}
  ...
  """
  @spec start_child_run(Jizoku.Step.Context.t(), module(), map(), keyword()) ::
          {:ok, Jizoku.ReadModel.Inspection.Snapshot.t()}
          | {:error, Config.config_error()}
  ...
  def start_child_run(parent_context, child_workflow, payload, overrides)
    ...
    with :ok <- Routing.public_child_start_options(overrides),
         ...
         {:ok, definition} <- Jizoku.Workflow.Definition.load(child_workflow) do
      child_trigger = Jizoku.Workflow.Definition.default_trigger(definition)
  ...
  @spec start_child_run(Jizoku.Step.Context.t(), module(), atom(), map(), keyword()) ::
          {:ok, Jizoku.ReadModel.Inspection.Snapshot.t()}
          | {:error, Config.config_error()}
          | {:error, {:invalid_option, atom() | term()}}
  ...
  @spec preview_graph_mutation(String.t(), map(), keyword()) ::
          {:ok, Jizoku.GraphMutation.Preview.t()} | {:error, term()}
  ...
  @spec apply_graph_mutation(String.t(), map(), keyword()) ::
          {:ok, Jizoku.GraphMutation.Report.t()} | {:error, term()}
  ...
  @spec reconcile_dynamic_graph(String.t(), keyword()) ::
          {:ok, Jizoku.GraphMutation.Reconciliation.t()} | {:error, term()}
  ...
  def reconcile_dynamic_graph(run_id, overrides) when is_list(overrides) do
    with :ok <- Routing.public_graph_reconciliation_options(overrides),
      ...
      {:ok,
       Jizoku.GraphMutation.Reconciliation.new(
  ...
  an already applied runnable; Jizoku uses the instruction ID as the durable
  ...
  @spec explain_run(String.t(), keyword()) ::
          {:ok, Jizoku.ReadModel.Explanation.Diagnostic.t()}
          | {:error,
             :not_found
             | :invalid_run_id
             | read_option_error()
             | Config.config_error()
             | Jizoku.ReadModel.Explanation.explanation_error()}
  ...
  def deliver_jido_signals(run_id, overrides)
    ...
    with :ok <- Routing.public_execute_options(overrides),
         {:ok, storage} <- Routing.journal_storage(overrides),
         {:ok, routes} <-
           Jizoku.Runtime.Jido.OutboxDelivery.routes(
             Keyword.get(
               overrides,
               :jido_dispatch_routes,
               Application.get_env(:jizoku, :jido_dispatch_routes)
             )
           ),
      ...
      Jizoku.Runtime.Jido.OutboxDelivery.deliver_run(storage, run_id, routes, now)
    else
  ...
  `config :jizoku, continuation_fences: :enabled` after every runtime worker
  has been upgraded to a fence-aware Jizoku release. `input:` and
  ...
  @doc """
  Applies a Jizoku-native or recognized Jido runtime command signal.
  ...
  boundary already has a `Jizoku.Runtime.Signal` or recognized `Jido.Signal`
  command envelope.
  ...
  a supported Jizoku command type. The signal source is audit provenance, not
  authorization; authenticate and authorize inbound signals before applying
  them. Domain signals require a host-owned `Jizoku.Jido.SignalResolver`
  ...
  defp schedule_idempotency_key(context) when is_map(context) do
    context
    |> Jizoku.Runtime.ScheduleContext.get()
    |> Jizoku.Runtime.ScheduleContext.value(:idempotency_key)
  ...
  defp schedule_run_id(workflow, trigger_name, idempotency_key) do
    workflow_name = Jizoku.Workflow.Definition.serialize_workflow(workflow)
    trigger = Jizoku.Workflow.Definition.serialize_trigger(trigger_name)
  ...
  defp executable_dynamic_work_attrs(
    ...
    Jizoku.Runtime.Jido.Instruction.dynamic_work(instruction, origin, registry)
  end
  ...
  defp record_dynamic_work_intent(
    ...
    case Jizoku.Runtime.Journal.append_entries(storage, [entry],
  ...
  defp schedule_dynamic_work_intent(
    ...
    with {:ok, planned_entry} <- dynamic_work_runnables_planned_entry(run_id, runnables, now),
         ...
         {:ok, _thread} <-
           Jizoku.Runtime.Journal.append_entries(storage, [entry, planned_entry],
             expected_rev: snapshot.thread_revisions.run
           ),
  ...
  defp trace_dynamic_work_intent(
    ...
    origin_trace =
      ...
      |> Enum.find(&(Jizoku.MapField.get(&1, :runnable_key) == origin_key))
      |> case do
        nil -> nil
        runnable -> Jizoku.MapField.get(runnable, :trace)
      end
  ...
  defp jido_instruction_metadata(node) do
    node
    |> Map.get(:metadata, %{})
    |> Jizoku.MapField.get(:jido_instruction)
  end
