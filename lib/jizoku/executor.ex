...
defmodule Jizoku.Executor do
  @moduledoc """
  Behaviour implemented by host applications to schedule Jizoku cron work.

  Jido-native execution is pulled through `Jizoku.execute_next/1`. Host
  applications that use external cron schedulers may still enqueue trigger
  activations as plain `Jizoku.Executor.Payload.cron/3` maps and deliver
  them to `Jizoku.Runtime.Runner.perform/2`.
  """
  ...
  can enqueue `Jizoku.Executor.Payload.cron/3` directly and deliver it to
  `Jizoku.Runtime.Runner.perform/2`.

  When the scheduler knows the logical schedule window, pass `:signal_id` and
  `:intended_window` through to `Jizoku.Executor.Payload.cron/3`. Jizoku
