...
defmodule Jizoku.Runtime.Jido.RunInstruction do
  @moduledoc false
  ...
  alias Jizoku.Runtime.Jido.Instruction
  ...
  @spec durable_entries(map(), ActionAttempt.t(), Agent.t(), map(), DateTime.t()) ::
          {:ok, [Jizoku.Runtime.DispatchProtocol.Entry.t()]}
          | {:error, error() | term()}
  def durable_entries(
  ...
  defp put_instruction_metadata(dynamic_work, node) do
    instruction = Jizoku.MapField.get(node.metadata, :jido_instruction)
