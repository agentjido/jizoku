defmodule Jizoku.Runtime.ScheduleMetadata do
  @moduledoc """
  ...
  declared cron trigger fires and queues a `Jizoku.Executor.Payload.cron/3`
  ...
  - when Jizoku actually received and started processing the signal
  ...
  alias Jizoku.Runtime.SchedulePayload
  ...
  `intended_window`. If the scheduler omits `signal_id`, Jizoku derives one
  ...
  complete intended window. Without that identity, Jizoku rejects the start
  because it cannot prove whether the activation is new or a duplicate.
  """
  @spec cron_context(module(), Jizoku.Workflow.Definition.trigger(), map()) ::
          {:ok, %{schedule: t()}}
  ...
  """
  @spec cron_context(module(), Jizoku.Workflow.Definition.trigger(), map(), DateTime.t()) ::
          {:ok, %{schedule: t()}}
          | {:error, {:invalid_schedule_signal_id, term()}}
          | {:error, {:invalid_schedule_intended_window, term()}}
          | {:error, {:missing_schedule_idempotency_key, atom()}}
  ...
  def cron_context(workflow, %{name: trigger_name, type: :cron, config: config}, payload, now)
    ...
    trigger_name = Jizoku.Workflow.Definition.serialize_trigger(trigger_name)
