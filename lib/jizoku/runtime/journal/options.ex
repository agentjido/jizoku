defmodule Jizoku.Runtime.Journal.Options do
  @moduledoc """
  ...
  boundary: caller-provided storage config is normalized into a Jizoku
  ...
  accepts the current Jido storage adapter shape through a Jizoku-owned
  ...
  """
  @spec storage(term()) ::
          {:ok, Jizoku.Runtime.Journal.Storage.t()} | {:error, {:invalid_option, term()}}
  def storage(storage), do: Jizoku.Runtime.Journal.Storage.normalize(storage)
  ...
  defp storage_partition(opts, storage) do
    case Keyword.fetch(opts, :partition) do
      ...
      :error -> {:ok, Jizoku.Runtime.Journal.Storage.partition(storage)}
    end
  end
