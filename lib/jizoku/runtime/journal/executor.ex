defmodule Jizoku.Runtime.Journal.Executor do
  @moduledoc """
  ...
  alias Jizoku.Runtime.BuiltInStep
  ...
  alias Jizoku.Runtime.Jido.EffectsActivation
  alias Jizoku.Runtime.Jido.Outbox
  alias Jizoku.Runtime.Jido.OutboxDelivery
  alias Jizoku.Runtime.Jido.ResultEnvelope
  alias Jizoku.Runtime.Jido.RunInstruction
  alias Jizoku.Runtime.Journal
  alias Jizoku.Runtime.Journal.Commands.ContinuationRecovery
  alias Jizoku.Runtime.Journal.Commands.NativeContinuation
  ...
  alias Jizoku.Runtime.Journal.Executor.ClaimContext
  alias Jizoku.Runtime.Journal.Executor.RuntimeContext
  ...
  alias Jizoku.Runtime.RetryPolicy
      ...
      interval_ms ->
        case heartbeat_claim(state) do
          ...
          {:lost, reason} ->
            ...
            exit({:jizoku_heartbeat_lost, reason})
        end
    end
  end
  ...
  defp execution_partition(opts) do
    case Options.storage_from_opts(opts) do
      {:ok, storage} -> Jizoku.Runtime.Journal.Storage.partition(storage)
      {:error, _reason} -> nil
    end
  end
  ...
  defp execute_step_and_record(
         %{
           storage: %{adapter: Jizoku.Runtime.Journal.Storage.Ecto, opts: storage_opts},
           step: %{opts: step_opts}
         } = execution
       )
  ...
  defp finish_repo_transaction_attempt(
         {:error, {:jizoku_step_error, reason}},
         execution,
         buffer
       ) do
    JournalEvents.discard(buffer)
  ...
  defp finish_repo_transaction_attempt(
         {:error, {:jizoku_transaction_completion_failed, reason}},
         _execution,
         buffer
       ) do
    JournalEvents.discard(buffer)
  ...
  defp record_transactional_step_result(
    ...
    repo.rollback({:jizoku_step_error, native_continuation_error(:unsupported_repo_transaction)})
  end
  ...
  defp record_transactional_step_result({:error, reason}, repo, _execution, mark_finishing) do
    mark_finishing.()
    repo.rollback({:jizoku_step_error, reason})
  end
  ...
  defp complete_transactional_attempt(repo, execution, output, execution_opts, guardrails) do
    with :ok <- run_test_after_transaction_step_hook(execution.opts, execution.claim.attempt),
      ...
      {:error, reason} ->
        repo.rollback({:jizoku_transaction_completion_failed, reason})
    end
  end
  ...
  defp run_transactional_step(%{module: module}, input, context) when is_atom(module) do
    if Step.native_step?(module) do
      ...
      {:error,
       %{
         message: "repo transaction steps must use a native Jizoku step",
         retryable?: false
       }}
    end
  end
  ...
  defp action_input(action, input) do
    if Jizoku.Step.native_step?(action) do
      {Jizoku.Step.Action, %{step: action, input: input}}
    else
      {action, input}
    end
  end
  ...
  defp owner_id(opts) do
    case Keyword.get(opts, :owner_id, "jizoku") do
