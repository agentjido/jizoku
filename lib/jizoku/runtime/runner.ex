defmodule Jizoku.Runtime.Runner do
  @moduledoc """
  ...
  is delivered. Step execution is claimed through `Jizoku.execute_next/1`.
  """
  ...
  `Jizoku.Executor.Payload` and pass the payload back here when the job is
  ...
  `Jizoku.Executor.Payload.cron/3` is persisted into the run context before
  ...
  defp existing_journal_schedule_start(workflow_name, trigger_name, signal_payload, overrides) do
    with {:ok, run_id} <- schedule_run_id(workflow_name, trigger_name, signal_payload),
         {:ok, %Snapshot{} = snapshot} <-
           Jizoku.inspect_run(run_id, journal_inspection_options(overrides)) do
  ...
  defp schedule_idempotency(context) when is_map(context) do
    ...
    |> Jizoku.Runtime.ScheduleContext.value(:idempotency)
    |> case do
