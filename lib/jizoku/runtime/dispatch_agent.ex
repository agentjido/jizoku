...
defmodule Jizoku.Runtime.DispatchAgent do
  @moduledoc """
  ...
  use Jido.Agent,
    name: "jizoku_dispatch_agent",
    description: "Rebuildable dispatch coordination state for one Jizoku queue.",
    default_plugins: false

  alias Jido.Agent
  alias Jizoku.Runtime.DispatchAgent.State
  alias Jizoku.Runtime.DispatchNotifier
  ...
  def agent_id(queue), do: "jizoku.dispatch.#{normalize_queue(queue)}"
  ...
  def agent_id(queue, partition) when is_binary(partition) do
    queue = normalize_queue(queue)
    "jizoku.dispatch.partition.#{Partition.identity([partition, queue])}"
  end
  ...
  @spec visible_attempts(Agent.t(), DateTime.t()) :: [
          Jizoku.Runtime.DispatchProtocol.ActionAttempt.t()
        ]
  ...
  """
  @spec completed_results(Agent.t()) :: [Jizoku.Runtime.DispatchProtocol.ActionAttempt.t()]
