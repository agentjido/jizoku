defmodule Jizoku.Runtime.WorkflowAgent do
  @moduledoc """
  ...
  use Jido.Agent,
    name: "jizoku_workflow_agent",
    description: "Rebuildable workflow coordination state for one Jizoku run.",
  ...
  def agent_id(run_id), do: "jizoku.workflow.#{run_id}"
  ...
  def agent_id(run_id, partition) when is_binary(run_id) and is_binary(partition) do
    "jizoku.workflow.partition.#{Partition.identity([partition, run_id])}"
  end
