defmodule Jizoku.Operations.Doctor do
  @moduledoc """
  ...
  defp configuration_check do
    misplaced =
      ...
      |> Enum.filter(&match?({:ok, _value}, Application.fetch_env(:jizoku, &1)))
      |> Enum.sort()

    case misplaced do
      [] ->
        check(:configuration, :pass, "Jizoku runtime configuration is valid.", [], %{})
  ...
  defp configuration_failure(reason) do
    check(
      ...
      [:configure_jizoku_repo_and_queue],
  ...
  defp schema_check(result) do
    {status, message} =
      case result.status do
        :current ->
          {:pass, "The host database matches Jizoku's required schema baseline."}

        :not_applicable ->
          {:pass, "Schema drift is not applicable to the configured storage adapter."}

        :behind ->
          {:fail, "The host database is behind Jizoku's required schema baseline."}

        :incompatible ->
          {:fail, "The host database has incompatible Jizoku schema objects."}

        :unavailable ->
          {:fail, "Jizoku could not inspect the host database schema."}
      end
  ...
  defp runtime_unavailable do
    check(
      :runtime_state,
      :fail,
      "Jizoku could not rebuild operational runtime projections.",
