defmodule Jizoku.Workflow.Validation do
  @moduledoc """
  ...
  `Jizoku.Workflow` can remain compact and declarative.
  """
  ...
  defp validate_transition_condition(errors, %{condition: condition, from: from}) do
    case Jizoku.Workflow.TransitionCondition.normalize(condition) do
      {:ok, _condition} ->
        errors
  ...
  defp duplicate_transition_condition?(transitions) do
    conditions =
      ...
      |> Enum.map(&Jizoku.Workflow.TransitionCondition.serialize(&1.condition))
