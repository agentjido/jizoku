...
defmodule Jizoku.Workflow.Spec do
  @moduledoc """
  ...
  defp transition_condition_key(transition) do
    case field(transition, :condition) do
      nil -> :unconditional
      condition -> {:condition, Jizoku.Workflow.TransitionCondition.serialize(condition)}
    end
  end
      ...
      on != :error ->
        [
          error(
            ...
            Jizoku.Workflow.ErrorDetails.transition_recovery(from, on, recovery)
          )
          | errors
        ]
  ...
  defp validate_transition_condition(errors, transition, index) when is_map(transition) do
    ...
    cond do
      ...
      match?({:ok, _condition}, Jizoku.Workflow.TransitionCondition.normalize(condition)) ->
        errors
