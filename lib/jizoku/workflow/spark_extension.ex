...
defmodule Jizoku.Workflow.SparkExtension do
  @moduledoc """
  Spark extension that defines the Jizoku workflow DSL.

  The extension owns trigger, payload, step, and transition declarations. It
  stores native `Jizoku.Step` metadata when available and marks built-in or
  ...
  @step_schema [
    ...
    module: [
      type: :atom,
      required: true,
      doc: "The native Jizoku step, raw Jido action, or built-in step kind."
    ],
    opts: [
      type: :keyword_list,
      default: [],
  ...
  @step %Spark.Dsl.Entity{
    name: :step,
    target: Jizoku.Workflow.StepSpec,
  ...
  @manual %Spark.Dsl.Entity{
    name: :manual,
    target: Jizoku.Workflow.TriggerDefinitionSpec,
    schema: [],
  ...
  @payload_field %Spark.Dsl.Entity{
    name: :field,
    target: Jizoku.Workflow.PayloadFieldSpec,
  ...
  @payload %Spark.Dsl.Entity{
    name: :payload,
    target: Jizoku.Workflow.PayloadSpec,
  ...
  @trigger %Spark.Dsl.Entity{
    name: :trigger,
    target: Jizoku.Workflow.TriggerSpec,
    args: [:name],
  ...
  @transition %Spark.Dsl.Entity{
    name: :transition,
    target: Jizoku.Workflow.TransitionSpec,
    args: [:from, :opts],
  ...
  @workflow %Spark.Dsl.Section{
    ...
    describe: "Declares Jizoku workflow triggers, steps, and transitions."
  }

  use Spark.Dsl.Extension, sections: [@workflow]

  @doc false
  @spec put_step_metadata(Jizoku.Workflow.StepSpec.t()) ::
          {:ok, Jizoku.Workflow.StepSpec.t()}
  def put_step_metadata(%Jizoku.Workflow.StepSpec{} = step) do
    metadata =
      case Jizoku.Step.metadata(step.module) do
  ...
  @doc false
  @spec put_cron_config(Jizoku.Workflow.TriggerDefinitionSpec.t()) ::
          {:ok, Jizoku.Workflow.TriggerDefinitionSpec.t()}
  def put_cron_config(%Jizoku.Workflow.TriggerDefinitionSpec{} = definition) do
    config = %{
  ...
  @doc false
  @spec reject_payload_field_outside_payload(Jizoku.Workflow.PayloadFieldSpec.t()) ::
          {:error, String.t()}
  def reject_payload_field_outside_payload(%Jizoku.Workflow.PayloadFieldSpec{}) do
    {:error, "field/3 must be declared inside a trigger payload block"}
  end

  @doc false
  @spec put_transition_options(Jizoku.Workflow.TransitionSpec.t()) ::
          {:ok, Jizoku.Workflow.TransitionSpec.t()} | {:error, String.t()}
  def put_transition_options(%Jizoku.Workflow.TransitionSpec{} = transition) do
  ...
  defp maybe_put_transition_recovery(%Jizoku.Workflow.TransitionSpec{opts: opts} = transition) do
  ...
  defp maybe_put_transition_condition(%Jizoku.Workflow.TransitionSpec{opts: opts} = transition) do
    case Keyword.fetch(opts, :condition) do
      {:ok, condition} ->
        %{
          transition
          | condition: Jizoku.Workflow.TransitionCondition.normalize!(condition)
        }
