...
defmodule Jizoku.Workflow.Definition do
  @moduledoc """
  Runtime-facing representation of a compiled workflow definition.

  `Jizoku.Workflow` builds the declarative DSL at compile time. This module
  ...
  transaction. Jizoku persists attempt, step, and run progression in its
  ...
  defp transition_matches?(%{condition: condition}, context) do
    Jizoku.Workflow.TransitionCondition.matches?(context, condition)
  end
  ...
  defp maybe_put_serialized_condition(serialized, condition) do
    case Jizoku.Workflow.TransitionCondition.serialize(condition) do
      nil -> serialized
  ...
  defp maybe_put_deserialized_condition(transition, condition) do
    case Jizoku.Workflow.TransitionCondition.deserialize(condition) do
      nil -> transition
  ...
  defp input_matches_type?(value, type), do: Jizoku.Workflow.ValueType.matches?(value, type)
