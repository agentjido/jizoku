...
defmodule Jizoku.Workflow.RunicPlanner do
  @moduledoc false
  ...
  @no_input {:jizoku, :no_planner_input}
  ...
  @doc """
  Builds a planner from a compiled Jizoku workflow module.
  """
  ...
  @doc """
  Applies the result of an externally executed Jizoku runnable.
  """
  ...
  defp stable_step_hash(spec, step) do
    :erlang.phash2({:jizoku_step, spec.workflow, step.name, step.module}, @max_phash)
  end
  ...
  defp prepare_external_runnables(%__MODULE__{} = planner) do
    ...
    if internal == [] and blocked == [] do
      ...
      with {:ok, runnables} <- to_jizoku_runnables(allowed, planner.spec) do
  ...
  defp to_jizoku_runnables(runnables, %Spec{} = spec) do
    result =
      Enum.reduce_while(runnables, {:ok, []}, fn runic_runnable, {:ok, acc} ->
        case to_jizoku_runnable(runic_runnable, spec) do
  ...
  defp to_jizoku_runnable(%Runic.Workflow.Runnable{} = runic_runnable, %Spec{} = spec) do
  ...
  defp fact_context(%Fact{meta: %{jizoku_context: context}}) when is_map(context) do
    context
  end

  defp fact_context(%Fact{value: value}), do: normalize_step_input(value)

  defp fact_outcome(%Fact{meta: %{jizoku_outcome: outcome}}) when outcome in [:ok, :error] do
    outcome
  end
  ...
  defp complete_runic_runnable(
    ...
    result_fact =
      Fact.new(
        ...
        meta: %{jizoku_context: next_context, jizoku_outcome: outcome}
      )
