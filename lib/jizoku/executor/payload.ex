defmodule Jizoku.Executor.Payload do
  @moduledoc """
  ...
  in job systems without depending on Jizoku structs or atoms. A queued job
  should deliver the stored payload back to `Jizoku.Runtime.Runner.perform/2`;
  ...
  - `:signal_id` - a stable scheduler signal id for this activation. When it is
    omitted, Jizoku derives a deterministic id from the trigger and intended
  ...
  def cron(workflow, trigger, opts \\ []) when is_atom(workflow) and is_list(opts) do
    %{
      "kind" => "cron",
      "workflow" => Jizoku.Workflow.Definition.serialize_workflow(workflow),
      "trigger" => Jizoku.Workflow.Definition.serialize_trigger(trigger)
    }
