...
defmodule Jizoku.Step do
  @moduledoc """
  Public contract for authoring native Jizoku workflow steps.

  Native steps return Jizoku values and receive a `Jizoku.Step.Context`.
  ...
  defmacro __using__(opts) do
    ...
    quote bind_quoted: [
      ...
      @behaviour Jizoku.Step

      @jizoku_step_name name
      @jizoku_step_description description
      @jizoku_step_input_schema input_schema
      @jizoku_step_output_schema output_schema

      @doc false
      def __jizoku_step__(:contract), do: :jizoku_step

      @doc false
      def __jizoku_step__(:metadata) do
        %{
          contract: :jizoku_step,
          name: @jizoku_step_name,
          description: @jizoku_step_description,
          input_schema: @jizoku_step_input_schema,
          output_schema: @jizoku_step_output_schema
        }
      end

      @doc false
      def __jizoku_step__(:input_schema), do: @jizoku_step_input_schema

      @doc false
      def __jizoku_step__(:output_schema), do: @jizoku_step_output_schema
    end
  end
  ...
  def native_step?(module) when is_atom(module) do
    Code.ensure_loaded?(module) and function_exported?(module, :__jizoku_step__, 1) and
      module.__jizoku_step__(:contract) == :jizoku_step
  end

  @doc false
  @spec metadata(module()) :: map() | nil
  def metadata(module) when is_atom(module) do
    if native_step?(module), do: module.__jizoku_step__(:metadata), else: nil
  end

  @doc false
  @spec validate_input(module(), map()) :: {:ok, map()} | {:error, map()}
  def validate_input(module, input) when is_atom(module) and is_map(input) do
    validate_schema(input, module.__jizoku_step__(:input_schema), "input")
  end
  ...
  def validate_output(module, output) when is_atom(module) and is_map(output) do
    validate_schema(output, module.__jizoku_step__(:output_schema), "output")
  end
  ...
  defp validate_schema_entries(value, schema, target) do
    ...
    case errors do
      ...
      errors ->
        {:error,
         Jizoku.Step.ErrorPayload.validation_failed(
