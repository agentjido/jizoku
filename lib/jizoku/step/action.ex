defmodule Jizoku.Step.Action do
  @moduledoc """
  Internal Jido action adapter for native Jizoku steps.

  Native step modules expose the public `Jizoku.Step` contract. This adapter
  preserves the runtime's Jido execution path by validating native input,
  converting the Jido context into `Jizoku.Step.Context`, normalizing native
  return values, and validating native output before the workflow runtime
  persists the result.
  """

  use Jido.Action,
    name: "jizoku_native_step",
    description: "Internal adapter for native Jizoku steps"
