defmodule Mix.Tasks.Jizoku.Status do
  @moduledoc """
  Reports read-only Jizoku run and queue health.
  ...
  @shortdoc "Reports read-only Jizoku run and queue health"

  use Mix.Task

  alias Jizoku.Operations.CLI
  alias Jizoku.Operations.Status

  @switches [json: :boolean]

  @impl Mix.Task
  def run(args) do
    opts = CLI.parse_options!("jizoku.status", args, @switches)
    ...
    case collect_report(opts) do
      ...
      {:error, reason} ->
        Mix.raise("Could not collect Jizoku status: #{CLI.format_error(reason)}")
    end
  end
  ...
  defp render_report(report, false) do
    Mix.shell().info("Jizoku status at #{report.generated_at}")
