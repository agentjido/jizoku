...
The public authoring surface is intentionally Jizoku-native: workflow
...
Jizoku sits between a job backend and a standalone workflow service.

- It is more than a job queue: Jizoku owns workflow structure, step state,
  attempts, retries, waits, approvals, replay policy, and inspection.
- It is less than a separate workflow platform: Jizoku runs inside the host
  app and keeps durable journal execution inside the host repo. Host workers
  call `Jizoku.execute_next/1`, and host schedulers may deliver cron
...
- Jizoku is not a generic replacement for all of them; it targets durable,
...
3. Jizoku records durable workflow and dispatch facts.
...
5. Optional backend adapters remain responsible for concrete delivery mechanics
   ...
   backend-owned leases today, while Jizoku keeps the core dispatch
...
- Out of scope: intentionally not part of Jizoku's product surface, even if
...
| Native step contract | Supported | `Jizoku.Step` is the preferred authoring path. Raw `Jido.Action` modules remain an explicit interop path. |
...
| Cron payload boundary | Supported | Host schedulers may enqueue `Jizoku.Executor.Payload.cron/3` maps and deliver them to `Jizoku.Runtime.Runner.perform/2`; step execution is claimed through `Jizoku.execute_next/1`. |
...
| Inspection and explanation | Supported, evolving | Journal-backed inspection and explanation are the default and infer Ecto storage from the configured repo; [#163](https://github.com/dark-trench/jizoku/issues/163) delivered the durable projection foundation. |
...
| Jido-native runtime agents | Supported, evolving | Workflow and dispatch agents rebuild from durable journals and checkpoints; [#164](https://github.com/dark-trench/jizoku/issues/164) covers the completed agent foundation. |
| Bedrock lease backend | Supported, evolving | The Bedrock example app demonstrates durable delivery, delayed visibility, leases, heartbeats, retries, and dead-letter behavior through Jizoku's backend-neutral lease boundary. |
...
| Conditional and deferred continuation | Supported, evolving | Durable planner facts and deferred wakeups are tracked in [#140](https://github.com/dark-trench/jizoku/issues/140). |
| Fan-out and fan-in contract | Supported, evolving | Runic-backed dependency ordering and join semantics are defined for the current static workflow graph; [#142](https://github.com/dark-trench/jizoku/issues/142) captured the closed design clarification. |
...
| UI graph serialization | Supported, evolving | `Jizoku.Runs.GraphInspection.to_map/1` exposes stable node, edge, status, output, and selected-edge data for dashboards and CLIs. `Jizoku.Workflow.EditorSpec` supports JSON-safe spec round trips and draft graph previews for visual editors. |
| Actor-scoped read views | Supported, evolving | `Jizoku.ReadModel.Visibility.redact/2` and `/3` let host apps derive external, operator, or auditor views from existing listing, inspection, graph, and explanation read models without mutating durable history. |
| Dynamic child runs | Supported, evolving | Native steps can start idempotent child workflow runs with durable parent lineage through `Jizoku.start_child_run/4` and `Jizoku.start_child_run/5`; visual-editor dynamic expansion is separate from the current child-run API. |
| Oban-specific core | Out of scope | Host apps may choose Oban behind the delivery boundary, but Jizoku core is not Oban-centric. |
| Exactly-once external side effects | Out of scope | Jizoku can provide durable workflow state and fencing semantics, but external systems still require idempotency. |
| Bundled workflow dashboard | Out of scope | Jizoku exposes inspection data; host apps own their operator UI. |

## Why Jizoku Exists Above Jido
...
operational runtime structure. Jizoku uses that foundation, but it is not
trying to make users write Jido-native workflow applications by hand.

Jizoku adds the workflow product layer:
...
agent. Use Jizoku when the main abstraction is a durable workflow run that
...
Jizoku intentionally sits one layer farther out. It treats the durable
...
| Project | Primary fit | Relationship to Jizoku |
| --- | --- | --- |
| [Jido](https://hex.pm/packages/jido) | OTP-native agents, actions, signals, directives, and supervised autonomous systems. | Runtime foundation and interop layer. Jizoku keeps raw Jido primitives out of the common workflow authoring path. |
| [Runic](https://hex.pm/packages/runic) | Data-driven workflow graphs, dependency planning, and runnable extraction. | Planner foundation. Jizoku maps declared workflow structure and readiness into durable runnable intent. |
| [Spark](https://hex.pm/packages/spark) | Declarative Elixir DSL and extension framework. | Authoring foundation. Jizoku uses Spark to define the workflow DSL and normalized workflow spec. |
| Durable backend adapters | Concrete delivery mechanics such as queues, scheduled work, leases, redelivery, and worker infrastructure. | Delivery foundation. Jizoku keeps this boundary backend-neutral and recommends Bedrock as the reference lease backend today. |

For application teams, these foundations are implementation boundaries rather
than prerequisites for the happy path. Use Jizoku APIs and workflow modules
...
| Project | Primary fit | Relationship to Jizoku |
| --- | --- | --- |
| [Reactor](https://hex.pm/packages/reactor) | Concurrent dependency-resolving saga orchestration for Elixir applications, with compensation and undo semantics. | Closest adjacent orchestrator. Jizoku emphasizes durable host-app workflow state, operator inspection, approvals, replay, cancellation, and backend-owned recovery. |
| [Ash Reactor](https://hexdocs.pm/ash/reactor.html) | Ash-native orchestration over resources and actions, built on Reactor. | Strong fit for Ash applications that want saga orchestration inside the Ash domain model. Jizoku targets broader Phoenix/OTP workflow runs with durable history, HITL, replay, cancellation, and recovery. |
| [Sage](https://hex.pm/packages/sage) | Dependency-free saga composition with transaction and compensation callbacks. | Good fit for local saga execution. Jizoku targets longer-lived inspectable workflow runs with persisted step and attempt history. |
| [FlowStone](https://hex.pm/packages/flowstone) | Asset-first data orchestration and dependency-aware pipelines. | Adjacent data-pipeline tool. Jizoku focuses on application workflows, approvals, recovery policy, dispatch semantics, and inspection. |
...
systems is now available through `Jizoku.start_spec/3` and
`Jizoku.start_spec/4`, with the safe action registry as the allowlist
...
`Jizoku.Workflow.EditorSpec`, and inspected run graph serialization is
available through `Jizoku.Runs.GraphInspection.to_map/1`.
...
It is an implementation detail, not the core Jizoku runtime model.
