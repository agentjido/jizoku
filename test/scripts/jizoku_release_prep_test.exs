defmodule Jizoku.ReleasePrepScriptTest do
  use ExUnit.Case, async: false

  @script Path.expand("../../scripts/jizoku_release_prep.exs", __DIR__)
  @command_env [{"GIT_TERMINAL_PROMPT", "0"}, {"GIT_ASKPASS", "true"}]

  setup do
    tmp_dir =
      Path.join(System.tmp_dir!(), "jizoku-release-prep-#{System.unique_integer([:positive])}")
  ...
  test "updates version metadata and changelog from commits since the previous tag", %{
    ...
    assert File.read!(Path.join(tmp_dir, "README.md")) =~ ~s({:jizoku, "~> 0.3.1"})
    ...
    assert host_app_integration =~ ~s({:jizoku, "~> 0.3.1"})
    refute host_app_integration =~ ~s({:jizoku, "~> 0.3.0"})
    ...
    assert output =~ "Prepared Jizoku 0.3.1 release metadata."
