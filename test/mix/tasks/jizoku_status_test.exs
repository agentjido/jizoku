defmodule Mix.Tasks.Jizoku.StatusTest do
  ...
  alias Mix.Tasks.Jizoku.Status

  @task "jizoku.status"
  @table :jizoku_status_task_test
  ...
  test "prints operator-readable run and queue summaries" do
    ...
    assert output =~ "Jizoku status at"
  ...
  test "rejects unknown options" do
    assert_raise Mix.Error, ~r/Invalid jizoku.status options/, fn ->
      Status.run(["--recover"])
    end
  end
