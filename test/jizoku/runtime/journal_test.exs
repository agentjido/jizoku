defmodule Jizoku.Runtime.JournalTest do
  ...
  @storage {ETS, table: :jizoku_journal_test}
  @run_id "run_123"
  ...
  test "appends runtime entries to Jido storage and rebuilds dispatch projections" do
    ...
    assert thread.id == "jizoku:dispatch:default"
    assert thread.rev == 1
  ...
  test "normalizes journal storage behind a Jizoku-owned boundary" do
    assert {:ok, %Storage{adapter: ETS, opts: [table: :jizoku_journal_test]}} =
  ...
  test "scopes durable thread and checkpoint identities by partition while preserving legacy ids" do
    assert Journal.thread_id({:run, @run_id}) == "jizoku:run:run_123"
    assert Journal.thread_id({:dispatch, "default"}) == "jizoku:dispatch:default"

    assert Journal.thread_id({:run_index, @workflow}) ==
             "jizoku:run_index:BillingWorkflow"

    assert Journal.thread_id({:run_catalog, "all"}) == "jizoku:run_catalog:all"

    assert Journal.thread_id({:run, @run_id}, "tenant_acme") ==
             "jizoku:partition:tenant_acme:run:run_123"

    assert Journal.thread_id({:dispatch, "default"}, "tenant_acme") ==
             "jizoku:partition:tenant_acme:dispatch:default"

    assert Journal.thread_id({:run_index, @workflow}, "tenant_acme") ==
             "jizoku:partition:tenant_acme:run_index:BillingWorkflow"

    assert Journal.thread_id({:run_catalog, "all"}, "tenant_acme") ==
             "jizoku:partition:tenant_acme:run_catalog:all"
    ...
    assert {:ok, %{id: "jizoku:partition:tenant_acme:dispatch:default"}} =
    ...
    assert {:ok, %Checkpoint{thread_id: "jizoku:partition:tenant_acme:dispatch:default"}} =
  ...
  test "revalidates normalized journal storage structs" do
    ...
    storage = %Storage{
      adapter: ETS,
      opts: [table: :jizoku_journal_test],
      config: caller_config
    }

    assert {:ok, %Storage{config: {ETS, [table: :jizoku_journal_test]}}} =
             Storage.normalize(storage)
  end
  ...
  test "loads thread metadata with decoded entries" do
    ...
    assert {:ok,
            %{
              ...
              thread_id: "jizoku:dispatch:default",
              rev: 2,
              entries: ^entries
  ...
  test "emits sanitized lifecycle points only after committed appends and in fact order" do
    ...
    assert_receive {:telemetry_event, [:jizoku, :runtime, :command, :received],
    ...
    assert_receive {:telemetry_event, [:jizoku, :runtime, :run, :started], %{count: 1},
                    started_metadata}
    ...
    assert_receive {:telemetry_event, [:jizoku, :runtime, :runnable, :planned], %{count: 1},
                    planned_metadata}
    ...
    for event <- [:scheduled, :claimed, :heartbeat, :completed] do
      assert_receive {:telemetry_event, [:jizoku, :runtime, :attempt, ^event], %{count: 1},
                      metadata}
  ...
  test "emits committed lifecycle points without rereading durable threads" do
    storage =
      {LoadTrackingStorage, table: :jizoku_journal_test, test_pid: self()}
    ...
    assert {:ok, %{rev: 1}} =
             Journal.append_entries(storage, [entry],
               expected_rev: 0,
               telemetry_projection: Jizoku.Runtime.WorkflowAgent.Projection.new()
             )
  ...
  test "emits failure and retry scheduling once without exposing errors" do
    failed_event = [:jizoku, :runtime, :attempt, :failed]
    retry_event = [:jizoku, :runtime, :attempt, :retry_scheduled]
  ...
  test "maps committed run lifecycle facts to exact sanitized point schemas" do
    ...
    workflow_projection = Jizoku.Runtime.WorkflowAgent.Projection.rebuild(run_entries)
    ...
    assert_receive {:telemetry_event, [:jizoku, :runtime, :runnable, :applied], %{count: 1},
                    applied_metadata}
    ...
    assert_receive {:telemetry_event, [:jizoku, :runtime, :manual, :paused], %{count: 1},
                    paused_metadata}

    assert paused_metadata.kind == "approval"
    assert paused_metadata.step == "charge_card"

    assert_receive {:telemetry_event, [:jizoku, :runtime, :manual, :resolved], %{count: 1},
                    resolved_metadata}

    assert resolved_metadata.action == "approved"
    assert resolved_metadata.step == "charge_card"

    assert_receive {:telemetry_event, [:jizoku, :runtime, :child, :started], %{count: 1},
                    child_metadata}

    assert child_metadata.child_run_id == @second_run_id

    assert_receive {:telemetry_event, [:jizoku, :runtime, :dynamic_work, :recorded], %{count: 1},
                    dynamic_metadata}

    assert dynamic_metadata.dynamic_key == "fanout-1"

    assert_receive {:telemetry_event, [:jizoku, :runtime, :run, :terminal], %{count: 1},
                    terminal_metadata}
  ...
  test "conflicts checkpoints and projection rebuilds emit no lifecycle points" do
    event = [:jizoku, :runtime, :attempt, :scheduled]
  ...
  test "buffers sanitized committed intents until explicit flush and discards them on demand" do
    started_event = [:jizoku, :runtime, :run, :started]
    planned_event = [:jizoku, :runtime, :runnable, :planned]
    terminal_event = [:jizoku, :runtime, :run, :terminal]
  ...
  defp table_name(:checkpoints), do: :jizoku_journal_test_checkpoints
  defp table_name(:threads), do: :jizoku_journal_test_threads
  defp table_name(:thread_meta), do: :jizoku_journal_test_thread_meta
