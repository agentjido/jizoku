defmodule Jizoku.Runtime.Journal.ContinuationPredecessorTest do
  ...
  @storage {ETS, table: :jizoku_continuation_predecessor_test}
  ...
  test "commits continuation request before the continued terminal in one predecessor append" do
    ...
    assert_receive {
      ...
      {"jizoku", :checkpoint, "jizoku:run:#{@run_id}"}
    }

    refute_receive {:storage_append, "jizoku:run:#{@run_id}", _kinds}
  ...
  test "rejects a terminal-only partial predecessor commit" do
    ...
    append_run_entry(
      Jizoku.Runtime.Journal.EntryBuilder.traced_run_terminal!(
        @run_id,
        :continued,
        @trace,
        @now
  ...
  defp cleanup_storage do
    for table <- [
          :jizoku_continuation_predecessor_test_checkpoints,
          :jizoku_continuation_predecessor_test_threads,
          :jizoku_continuation_predecessor_test_thread_meta
        ] do
      delete_table(table)
    end
  end
