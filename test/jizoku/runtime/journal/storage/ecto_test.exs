defmodule Jizoku.Runtime.Journal.Storage.EctoTest do
  ...
  defmodule MutationOriginAction do
    use Jizoku.Step, name: :mutation_origin
  ...
  defmodule MutationAddedAction do
    use Jizoku.Step, name: :mutation_added
  ...
  defmodule MutationHoldAction do
    use Jizoku.Step, name: :mutation_hold
  ...
  defmodule MutationWorkflow do
    ...
    alias Jizoku.Runtime.Journal.Storage.EctoTest.MutationHoldAction
    alias Jizoku.Runtime.Journal.Storage.EctoTest.MutationOriginAction
  ...
  @storage_adapter Jizoku.Runtime.Journal.Storage.Ecto

  @storage {@storage_adapter, repo: Repo}
  @thread_id "jizoku:dispatch:ecto-storage"
  @run_id "run_123"
  ...
  test "fails closed when a persisted entry would create a new atom" do
    ...
    entry =
      :attempt_scheduled
      |> entry(%{run_id: @run_id})
      |> Map.put(:refs, %{jizoku_thread: {:dispatch, "default"}})

    encoded_entry =
      entry
      |> :erlang.term_to_binary()
      |> String.replace("jizoku_thread", unknown_atom_name)
  ...
  test "round-trips checkpoints by arbitrary key term" do
    key = {"jizoku", :checkpoint, @thread_id}
  ...
  test "integrates with the Jizoku journal boundary" do
  ...
  defp versioned_binary(encoded_value) do
    :erlang.term_to_binary({:jizoku_ecto_term_v1, encoded_value})
  end
end
