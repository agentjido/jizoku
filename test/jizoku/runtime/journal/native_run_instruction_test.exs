defmodule Jizoku.Runtime.Journal.NativeRunInstructionTest do
          ...
          corrupted_entries =
            Enum.map(entries, fn entry ->
              payload =
                put_in(
                  entry.payload,
                  [:data, :result, "__jizoku_jido_result__", "fingerprint"],
                  "corrupted"
                )
  ...
  defmodule Seed do
    use Jizoku.Step, name: :native_run_instruction_seed
  ...
  defmodule RecoverCollision do
    use Jizoku.Step, name: :native_run_instruction_collision_recovery
  ...
  @storage {ETS, table: :jizoku_native_run_instruction_test}
  ...
  test "a committed completion is repaired without rerunning the source action" do
    ...
    assert %{"__jizoku_jido_result__" => %{"kind" => "run_instruction"}} = result
    ...
    legacy_projection =
      ...
      |> Map.delete("jizoku_dispatch_checkpoint_version")
  ...
  defp restore_activation({:ok, value}) do
    Application.put_env(:jizoku, :jido_effects, value)
  end
  ...
  defp cleanup_storage do
    for table <- [
          :jizoku_native_run_instruction_test_checkpoints,
          :jizoku_native_run_instruction_test_threads,
          :jizoku_native_run_instruction_test_thread_meta
        ] do
  ...
  defp delete_checkpoint(thread) do
    ...
    adapter.delete_checkpoint(
      {"jizoku", :checkpoint, Journal.thread_id(thread)},
      opts
    )
  end
end
