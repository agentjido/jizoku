defmodule Jizoku.Runtime.DispatchAgentTest do
  ...
  @storage {ETS, table: :jizoku_dispatch_agent_test}
  @run_id "run_123"
  ...
  test "rebuilds a keyed dispatch agent from durable dispatch entries" do
    ...
    assert agent.id == "jizoku.dispatch.default"
  ...
  test "partitioned dispatch agents have isolated collision-safe identities" do
    assert DispatchAgent.agent_id("default", nil) == "jizoku.dispatch.default"
  ...
  test "rejects lifecycle appends with a stale claim token before writing" do
    ...
    completed_event = [:jizoku, :runtime, :attempt, :completed]
    Jizoku.Test.TelemetryCapture.attach([completed_event])
  ...
  test "returns an error when a related run thread has incompatible persisted entries" do
    ...
    assert {:ok, _thread} =
             ETS.append_thread(
               ...
               [%{kind: :note, payload: %{}}],
               table: :jizoku_dispatch_agent_test
             )
  ...
  defp table_name(:checkpoints), do: :jizoku_dispatch_agent_test_checkpoints
  defp table_name(:threads), do: :jizoku_dispatch_agent_test_threads
  defp table_name(:thread_meta), do: :jizoku_dispatch_agent_test_thread_meta
