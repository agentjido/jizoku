defmodule Jizoku.ReadModel.InspectionTest do
  ...
  @storage {Jido.Storage.ETS, table: :jizoku_read_model_inspection_test}
  @run_id "run_123"
  ...
  test "exposes only public completion results for encoded and ordinary attempts" do
    ...
    refute inspect(snapshot) =~ "__jizoku_jido_result__"
    ...
    malformed =
      put_in(
        envelope,
        ["__jizoku_jido_result__", "fingerprint"],
        "malformed"
      )
    ...
    cleanup_storage()
    append_run_entries([run_started(), runnables_planned()])
    ordinary = %{"__jizoku_jido_result__" => %{application: "ordinary"}}
  ...
  defp table_name(:checkpoints), do: :jizoku_read_model_inspection_test_checkpoints
  defp table_name(:threads), do: :jizoku_read_model_inspection_test_threads
  defp table_name(:thread_meta), do: :jizoku_read_model_inspection_test_thread_meta
