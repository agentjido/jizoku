defmodule Jizoku.Operations.DoctorTest do
  ...
  @storage {Jido.Storage.ETS, table: :jizoku_doctor_findings_test}

  setup do
    :ok = Jido.Storage.ETS.Owner.ensure_tables(table: :jizoku_doctor_findings_test)
    on_exit(&drop_tables/0)
    :ok
  end

  test "reports an empty custom-storage runtime as healthy and JSON-safe" do
    storage = {Jido.Storage.ETS, table: :jizoku_doctor_empty_test}
  ...
  test "turns invalid configuration into an actionable diagnostic" do
    ...
    assert [
             %{
               id: :configuration,
               status: :fail,
               next_actions: [:configure_jizoku_repo_and_queue],
  ...
  test "reports missing configuration without exposing configuration values" do
    ...
    assert [
             %{
               id: :configuration,
               status: :fail,
               message: "Missing Jizoku configuration: :journal_storage.",
  ...
  defp drop_tables do
    ...
    |> Enum.map(&String.to_existing_atom("jizoku_doctor_findings_test_#{&1}"))
    |> Enum.each(fn table ->
