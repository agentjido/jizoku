defmodule Jizoku.Jido.SignalResolverTest do
  ...
  defmodule RecordOrder do
    use Jizoku.Step, name: :record_resolved_order
  ...
  defmodule Resolver do
    ...
    def resolve(%Jido.Signal{type: "orders.created", data: %{"order_id" => order_id}}) do
      {:ok,
       {:start_run, Jizoku.Jido.SignalResolverTest.OrderWorkflow, :domain_signal,
        %{order_id: order_id}}}
    end
  ...
  test "routes a validated domain signal into an idempotent durable start", %{
    ...
    assert {:error, {:conflicting_jido_signal, :envelope}} =
             Jizoku.apply_signal(conflicting, runtime_opts(storage, Resolver))
  ...
  test "treats source and id together as the partition-scoped event identity", %{
    ...
    assert {:ok, first} = Jizoku.apply_signal(first_signal, runtime_opts(storage, Resolver))
    assert {:ok, second} = Jizoku.apply_signal(second_signal, runtime_opts(storage, Resolver))
  ...
  test "serializes concurrent resolver decisions through one durable ingress winner", %{
    ...
    second =
      Task.async(fn ->
        Jizoku.apply_signal(signal, runtime_opts(storage, AlternateResolver))
      end)
  ...
  test "rejects the losing envelope in a concurrent identity conflict", %{server: server} do
    ...
    second =
      Task.async(fn ->
        Jizoku.apply_signal(changed, runtime_opts(storage, Resolver))
      end)
  ...
  test "isolates the same event identity across runtime partitions", %{
    ...
    assert {:ok, first} =
             Jizoku.apply_signal(signal, runtime_opts(storage, Resolver, partition: "tenant_a"))

    assert {:ok, second} =
             Jizoku.apply_signal(signal, runtime_opts(storage, Resolver, partition: "tenant_b"))
  ...
  test "fails closed for missing, invalid, rejected, and unsafe resolver results", %{
    ...
    assert {:error, {:invalid_signal_adapter, {:type, :unsupported}}} =
             Jizoku.apply_signal(signal, runtime_opts(storage))

    assert {:error, {:invalid_option, {:jido_signal_resolver, :invalid}}} =
             Jizoku.apply_signal(signal, runtime_opts(storage, NotAResolver))

    assert {:error, {:invalid_jido_signal_command, :unsupported}} =
             Jizoku.apply_signal(signal, runtime_opts(storage, StringWorkflowResolver))

    rejected = %{signal | type: "orders.unknown"}

    assert {:error, {:jido_signal_rejected, :unrouted}} =
             Jizoku.apply_signal(rejected, runtime_opts(storage, Resolver))

    assert {:error, {:jido_signal_resolver_failed, :exception}} =
             Jizoku.apply_signal(signal, runtime_opts(storage, RaisingResolver))

    refute inspect(Jizoku.apply_signal(signal, runtime_opts(storage, RaisingResolver))) =~
             "resolver secret"
  ...
  test "validates the Jido envelope before invoking the host resolver", %{
    ...
    for invalid <- invalid_signals do
      assert {:error, {:invalid_signal_adapter, _reason}} =
               Jizoku.apply_signal(invalid, runtime_opts(storage, RaisingResolver))
    end
  ...
  test "isolates resolver throw, exit, and malformed result failures", %{
    ...
    assert {:error, {:jido_signal_resolver_failed, :throw}} =
             Jizoku.apply_signal(signal, runtime_opts(storage, ThrowingResolver))

    assert {:error, {:jido_signal_resolver_failed, :exit}} =
             Jizoku.apply_signal(signal, runtime_opts(storage, ExitingResolver))

    assert {:error, {:invalid_jido_signal_command, :unsupported}} =
             Jizoku.apply_signal(signal, runtime_opts(storage, InvalidResultResolver))

    assert {:error, {:invalid_jido_signal_command, :invalid_resolver_result}} =
             Jizoku.apply_signal(signal, runtime_opts(storage, MalformedResultResolver))

    assert {:error, {:invalid_jido_signal_command, :invalid_resolver_result}} =
             Jizoku.apply_signal(signal, runtime_opts(storage, NonAtomRejectionResolver))

    refute inspect(Jizoku.apply_signal(signal, runtime_opts(storage, ThrowingResolver))) =~
             "resolver_secret"
