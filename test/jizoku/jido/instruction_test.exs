defmodule Jizoku.Jido.InstructionTest do
  ...
  defmodule PrepareOrder do
    use Jizoku.Step, name: :prepare_instruction_origin
  ...
  defmodule FinishOrder do
    use Jizoku.Step, name: :finish_instruction_workflow
  ...
  test "requires one applied durable origin without mutating the run" do
    ...
    assert {:error, {:invalid_jido_instruction, {:origin, :required}}} =
             Jizoku.schedule_dynamic_work(run.run_id, instruction(), runtime_opts(runtime))
