defmodule Jizoku.Jido.OutboxDeliveryTest do
  ...
  test "execute delivery acknowledges a terminal emit without exposing its payload" do
    ...
    assert_receive {:telemetry_event, [:jizoku, :runtime, :jido_signal, :enqueued], _, metadata}
    assert metadata.signal_id == "delivery-signal-1"
    assert metadata.route == "default"
    refute inspect(metadata) =~ "payload-secret"

    assert_receive {:telemetry_event, [:jizoku, :runtime, :jido_signal, :deliver, :start], _, _}
    assert_receive {:telemetry_event, [:jizoku, :runtime, :jido_signal, :deliver, :stop], _, _}
    assert_receive {:telemetry_event, [:jizoku, :runtime, :jido_signal, :delivered], _, _}
  ...
  test "dispatch failure stays pending and returns only structural diagnostics" do
    ...
    assert {:error, {:jido_signal_delivery_failed, failure}} =
             Jizoku.deliver_jido_signals(run.run_id,
    ...
    assert_receive {:telemetry_event, [:jizoku, :runtime, :jido_signal, :deliver, :stop], _,
                    metadata}
