defmodule Jizoku.GraphMutationApplyTest do
  use ExUnit.Case, async: false

  alias Jizoku.GraphMutation.Reconciliation
  ...
  @storage {Jido.Storage.ETS, table: :jizoku_graph_mutation_apply_test}
  ...
  test "atomically commits mutation intent and returns exact duplicates" do
    assert {:ok, %Report{} = report} =
             Jizoku.apply_graph_mutation(@run_id, mutation(), apply_options())
    ...
    assert {:ok, %Report{} = duplicate} =
             Jizoku.apply_graph_mutation(@run_id, mutation(), duplicate_options)
    ...
    assert Jizoku.apply_graph_mutation(@run_id, conflicting, apply_options()) ==
             {:error, {:invalid_graph_mutation, {:mutation_id, {:conflict, "mutation-apply"}}}}

    stale = %{mutation() | mutation_id: "mutation-stale"}

    assert Jizoku.apply_graph_mutation(@run_id, stale, apply_options()) ==
  ...
  test "reports committed dispatch failure and public reconciliation repairs it" do
    ...
    assert {:ok, %Report{} = report} =
             Jizoku.apply_graph_mutation(@run_id, mutation(), options)
    ...
    pending_payload = Jizoku.Runs.GraphInspection.to_map(pending_graph)
    ...
    assert {:ok, %Reconciliation{} = reconciliation} =
             Jizoku.reconcile_dynamic_graph(@run_id,
               runtime: :journal,
               journal_storage: @storage,
               queue: @queue,
               now: @now
  ...
  test "failed run append creates no mutation or dynamic dispatch facts" do
    ...
    assert Jizoku.apply_graph_mutation(@run_id, mutation(), options) ==
             {:error, :append_failed}
  ...
  test "rejects unsupported and malformed public options" do
    assert Jizoku.apply_graph_mutation(@run_id, mutation(), unknown: true) ==
             {:error, {:invalid_option, {:option, :unknown}}}

    assert Jizoku.apply_graph_mutation(@run_id, mutation(), :invalid) ==
             {:error, {:invalid_option, {:opts, :invalid}}}

    assert Jizoku.reconcile_dynamic_graph(@run_id, limits: %{}) ==
             {:error, {:invalid_option, {:option, :limits}}}

    assert Jizoku.reconcile_dynamic_graph(@run_id, :invalid) ==
             {:error, {:invalid_option, {:opts, :invalid}}}
  end
  ...
  defp storage_tables do
    [
      :jizoku_graph_mutation_apply_test_checkpoints,
      :jizoku_graph_mutation_apply_test_threads,
      :jizoku_graph_mutation_apply_test_thread_meta
    ]
  end
end
