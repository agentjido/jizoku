defmodule Jizoku.RuntimeSpecStartTest do
  use ExUnit.Case, async: false

  defmodule LoadInvoice do
    use Jizoku.Step, name: :load_invoice
  ...
  defmodule SendReminder do
    use Jizoku.Step, name: :send_reminder
  ...
  defmodule HandleGuardrailFailure do
    use Jizoku.Step, name: :handle_guardrail_failure
  ...
  defmodule ElixirAdapters do
    @spec load_invoice(map(), Jizoku.Step.Context.t()) :: {:ok, map()}
  ...
  @storage {Jido.Storage.ETS, table: :jizoku_runtime_spec_start_test}
  ...
  test "starts and executes a validated runtime-authored spec" do
    ...
    assert {:ok, first} =
             Jizoku.execute_next(journal_storage: @storage, owner_id: "worker")
    ...
    assert {:ok, inspected} = Jizoku.inspect_run(@run_id, journal_storage: @storage)
  ...
  test "rejects unknown action keys before creating a run" do
    assert {:error, {:invalid_workflow_spec, errors}} =
             Jizoku.start_spec(runtime_invoice_spec(), %{invoice_id: "inv_123"},
               action_registry: %{},
               journal_storage: @storage,
    ...
    assert {:error, :not_found} =
             Jizoku.inspect_run(@missing_action_run_id, journal_storage: @storage)
  end
  ...
  test "blocks runtime-authored run start when an input guardrail fails" do
    ...
    assert {:error, :not_found} =
             Jizoku.inspect_run(@guardrail_blocked_run_id, journal_storage: @storage)
  end
  ...
  test "routes runtime-authored guardrail failures through explicit error policy" do
    ...
    assert {:ok, explanation} =
             Jizoku.explain_run(@guardrail_routed_run_id, journal_storage: @storage)
  ...
  test "keeps passed runtime guardrail decisions durable after runnable application" do
    ...
    assert {:ok, inspected} =
             Jizoku.inspect_run(@guardrail_passed_run_id, journal_storage: @storage)
    ...
    assert {:ok, explanation} =
             Jizoku.explain_run(@guardrail_passed_run_id, journal_storage: @storage)
  ...
  test "rejects invalid named triggers without raising" do
    assert {:error, {:invalid_trigger, "manual"}} =
             Jizoku.start_spec(runtime_invoice_spec(), "manual", %{invoice_id: "inv_123"},
               action_registry: action_registry(),
               journal_storage: @storage
             )
  end
  ...
  test "rejects runtime-authored spec replays explicitly" do
    ...
    assert {:error, {:invalid_replay_source, :runtime_spec}} =
             Jizoku.replay(run_id, journal_storage: @storage)
  end
  ...
  test "executes an approved HTTP action key from a runtime-authored spec" do
    ...
    assert {:ok, completed} =
             Jizoku.execute_next(journal_storage: @storage, owner_id: "http-worker")
  ...
  test "rejects invalid HTTP action config before creating a runtime-authored run" do
    ...
    assert {:error, :not_found} =
             Jizoku.inspect_run(@invalid_http_action_run_id, journal_storage: @storage)
  end
  ...
  test "rejects invalid Elixir action adapters before creating a runtime-authored run" do
    ...
    assert {:error, :not_found} =
             Jizoku.inspect_run(@invalid_elixir_action_run_id, journal_storage: @storage)
  end
  ...
  defp assert_persisted_elixir_action_opts_are_safe(run_id) do
    assert {:ok, %{entries: entries}} =
             Jizoku.Runtime.Journal.load_thread(@storage, {:run, run_id})
  ...
  defp table_name(:checkpoints), do: :jizoku_runtime_spec_start_test_checkpoints
  defp table_name(:threads), do: :jizoku_runtime_spec_start_test_threads
  defp table_name(:thread_meta), do: :jizoku_runtime_spec_start_test_thread_meta
