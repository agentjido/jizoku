defmodule JizokuTest do
  ...
  alias Jizoku.Runtime.Journal.SignalInterpreter
  ...
  def saga_compensation_log_key(order_id), do: {:jizoku_test_saga_compensations, order_id}
  ...
  defmodule CommitThenFailStorage do
    ...
    @impl Jido.Storage
    def load_thread("jizoku:dispatch:" <> _queue, _opts), do: :not_found
  ...
  defmodule RepoTransactionWorkflow.RecordEvent do
    ...
    @impl Jizoku.Step
    def run(%{account_id: account_id}, %Jizoku.Step.Context{run_id: run_id}) do
      now = NaiveDateTime.utc_now(:second)

      Jizoku.Test.Repo.insert_all("transactional_events", [
        %{
  ...
  defmodule RepoTransactionContinuationWorkflow.RecordAndContinue do
    ...
    @impl Jizoku.Step
    def run(%{account_id: account_id} = input, %Jizoku.Step.Context{run_id: run_id}) do
      now = NaiveDateTime.utc_now(:second)
  ...
  defmodule ChildDigestWorkflow.FlakyDeliverDigest do
    ...
    @impl Jizoku.Step
    def run(%{subscription_id: subscription_id}, %Jizoku.Step.Context{attempt: 1}) do
      {:retry, %{message: "temporary digest failure", subscription_id: subscription_id}}
    end

    def run(%{subscription_id: subscription_id}, %Jizoku.Step.Context{attempt: 2}) do
      {:ok, %{delivered: %{subscription_id: subscription_id}}}
    end
  end

  defmodule DynamicElixirAdapters do
    @spec deliver(map(), Jizoku.Step.Context.t()) :: {:ok, map()}
  ...
  defmodule JournalRetryThenCompleteWorkflow.RetryGateway do
    ...
    @impl Jizoku.Step
    def run(%{account_id: _account_id}, %Jizoku.Step.Context{attempt: 1}) do
      {:retry, %{message: "gateway timeout"}}
    end

    def run(%{account_id: _account_id}, %Jizoku.Step.Context{attempt: 2}) do
      {:ok, %{gateway: "ok"}}
    end
  end
  ...
  defmodule JournalSagaRollbackWorkflow.ReleaseInventory do
    ...
    def run(
      ...
      JizokuTest.record_saga_compensation(order_id, :release_inventory)
      {:ok, %{released_inventory: Map.put(reservation, :status, "released")}}
    end
  end
  ...
  defmodule JournalSagaRollbackWorkflow.VoidPaymentAuthorization do
    ...
    def run(
      ...
      JizokuTest.record_saga_compensation(order_id, :void_payment_authorization)
  ...
  test "configures an application supervisor" do
    assert Application.spec(:jizoku, :mod) == {Jizoku.Application, []}
  end
  ...
  describe "config/1" do
    test "returns the validated host app contract with defaults" do
      assert {:ok, config} = Jizoku.config(repo: Jizoku.Test.Repo)

      assert config.repo == Jizoku.Test.Repo
      ...
      assert config.journal_storage.opts == [repo: Jizoku.Test.Repo]
    ...
    test "allows host applications to configure journal runtime defaults" do
      journal_storage = {Jido.Storage.ETS, table: :jizoku_config_test}
    ...
    test "preserves an explicitly scoped storage partition over the global default" do
      storage = {Jido.Storage.ETS, table: :jizoku_scoped_storage_precedence_test}
      ...
      assert {:ok, scoped_storage} =
               Jizoku.Runtime.Journal.Storage.scope(storage, "tenant_explicit")

      assert {:ok, resolved_storage} =
               Jizoku.Runtime.Routing.journal_storage(journal_storage: scoped_storage)

      assert Jizoku.Runtime.Journal.Storage.partition(resolved_storage) == "tenant_explicit"

      assert {:ok, %{partition: "tenant_explicit"}} =
               Jizoku.config(journal_storage: scoped_storage)

      assert Jizoku.Runtime.Routing.journal_start_options(journal_storage: scoped_storage)[
               :partition
             ] == "tenant_explicit"
    end

    test "allows explicit journal storage without a configured repo" do
      journal_storage = {Jido.Storage.ETS, table: :jizoku_config_no_repo_test}
      original_repo = Application.get_env(:jizoku, :repo)

      on_exit(fn ->
        if is_nil(original_repo) do
          Application.delete_env(:jizoku, :repo)
        else
          Application.put_env(:jizoku, :repo, original_repo)
        end
      end)
      ...
      assert config.read_model == :read_model
      assert config.journal_storage.adapter == Jido.Storage.ETS
      assert config.journal_storage.opts == [table: :jizoku_config_no_repo_test]
      assert config.queue == "configured_queue"
    end
    ...
    test "infers Ecto journal storage from the configured repo when runtime uses the journal" do
      ...
      assert {:ok, config} = Jizoku.config(Keyword.put(required, :runtime, :journal))
    ...
    test "infers Ecto journal storage from the configured repo when read model uses the journal" do
      ...
      assert {:ok, config} = Jizoku.config(Keyword.put(required, :read_model, :read_model))
    ...
    test "rejects explicit nil journal storage when configured runtime or read model uses the journal" do
      ...
      assert {:error, {:missing_config, [:journal_storage]}} =
               Jizoku.config(Keyword.put(required, :runtime, :journal))

      assert {:error, {:missing_config, [:journal_storage]}} =
               Jizoku.config(Keyword.put(required, :read_model, :read_model))
    end
    ...
    test "reports missing required configuration keys" do
      ...
      assert {:error, {:missing_config, [:repo]}} = Jizoku.config()

      assert_raise ArgumentError,
                   ~r/config :jizoku, repo: .*journal_storage:/,
                   fn ->
                     Jizoku.config!()
                   end
    end
    ...
    test "reports invalid repo configuration separately from missing repo config" do
      assert {:error, {:invalid_config, [repo: :invalid]}} =
               Jizoku.config(repo: "not_a_repo")
    end

    test "journal-only configuration still rejects unsupported runtimes" do
      assert {:error, {:invalid_config, [runtime: :unsupported]}} =
               Jizoku.config(repo: Jizoku.Test.Repo, runtime: :unsupported)
    end
  end
  ...
  describe "journal-only runtime payloads" do
    ...
    test "inspect_run_timeline/2 returns chronological redaction-safe operator events" do
      ...
      assert {:ok, %Snapshot{} = started} =
               Jizoku.start(BillingWorkflow, :billing, %{payment_id: "pay_secret_123"},
                 now: started_at
               )

      assert {:ok, %Snapshot{} = _completed_step} =
               Jizoku.execute_next(owner_id: "timeline-worker", now: executed_at)

      assert {:ok, timeline} = Jizoku.inspect_run_timeline(started.run_id, now: executed_at)

      assert timeline.run_id == started.run_id
      assert timeline.workflow == "Elixir.JizokuTest.BillingWorkflow"
    ...
    test "cron starts run through the journal default and expose schedule context" do
      storage = {Jido.Storage.ETS, table: :jizoku_journal_cron_context_test}
      ...
      assert {:ok, [%Summary{} = summary]} = Jizoku.list_runs([])

      assert {:ok, %Snapshot{} = started} =
               Jizoku.inspect_run(summary.run_id, now: started_at)
      ...
      assert [
               %{
                 ...
                 payload: %{
                   workflow: "Elixir.JizokuTest.ScheduledContextWorkflow",
    ...
    test "cron payloads preserve partition identity across durable delivery" do
      storage = {Jido.Storage.ETS, table: :jizoku_partitioned_cron_test}
    ...
    test "apply_signal/2 starts cron-triggered journal runs from command signals" do
      storage = {Jido.Storage.ETS, table: :jizoku_journal_cron_signal_test}
    ...
    test "idempotent cron starts reuse one journal run for duplicate schedule delivery" do
      storage = {Jido.Storage.ETS, table: :jizoku_journal_cron_idempotency_test}
      ...
      assert {:ok, %Snapshot{} = started} =
               Jizoku.inspect_run(summary.run_id,
                 runtime: :journal,
                 read_model: :read_model,
                 journal_storage: storage,
                 queue: queue,
    ...
    test "duplicate journal cron starts survive queue changes for the same schedule identity" do
      storage = {Jido.Storage.ETS, table: :jizoku_journal_cron_duplicate_queue_change_test}
      started_at = ~U[2026-05-15 00:00:00Z]
    ...
    test "duplicate journal cron starts survive current workflow definition drift" do
      storage =
        {Jido.Storage.ETS, table: :jizoku_journal_cron_duplicate_definition_drift_test}
      ...
      assert {:ok, [%Summary{} = summary]} = Jizoku.list_runs([], opts)
    ...
    test "duplicate journal cron starts derive identity after workflow definition drift" do
      storage = {Jido.Storage.ETS, table: :jizoku_journal_cron_duplicate_derived_drift_test}
      started_at = ~U[2026-05-15 00:00:00Z]
    ...
    test "journal cron duplicate classification ignores step output schedule keys" do
      storage = {Jido.Storage.ETS, table: :jizoku_journal_cron_schedule_clobber_test}
    ...
    test "replay/2 preserves journal cron schedule context" do
      storage = {Jido.Storage.ETS, table: :jizoku_journal_cron_replay_context_test}
    ...
    test "replay/2 removes schedule idempotency identity from journal cron context" do
      storage = {Jido.Storage.ETS, table: :jizoku_journal_cron_replay_idempotency_test}
    ...
    test "journal cron starts reject malformed schedule idempotency keys" do
      assert {:error, {:invalid_option, {:schedule_idempotency_key, :invalid}}} =
               Jizoku.start_run_with_initial_context(
                 IdempotentScheduledContextWorkflow,
                 :scheduled_capture,
                 %{},
                 %{schedule: %{idempotency_key: 123}},
                 runtime: :journal,
                 journal_storage: {Jido.Storage.ETS, table: :jizoku_journal_cron_bad_key_test},
      ...
      assert {:error, {:invalid_option, {:schedule_idempotency_key, :invalid}}} =
               SignalInterpreter.apply(signal,
                 journal_storage: {Jido.Storage.ETS, table: :jizoku_journal_cron_empty_key_test},
    ...
    test "journal cron starts return structured option errors" do
      assert {:error, {:invalid_option, {:queue, :invalid}}} =
               Jizoku.start_run_with_initial_context(
                 ...
                 runtime: :journal,
                 journal_storage: {Jido.Storage.ETS, table: :jizoku_journal_cron_bad_queue_test},
                 queue: ""
               )
    end

    test "malformed journal cron scheduler metadata does not create a run" do
      storage = {Jido.Storage.ETS, table: :jizoku_journal_cron_invalid_metadata_test}
  ...
  defp dynamic_cron_workflow_source(module, :with_idempotent_schedule) do
    ...
    defmodule #{inspect(module)} do
      ...
      workflow do
        ...
        step :capture_schedule, JizokuTest.ScheduledContextWorkflow.CaptureSchedule
        transition :capture_schedule, on: :ok, to: :complete
      end
    end
    """
  ...
  describe "read model" do
    @read_model_storage {Jido.Storage.ETS, table: :jizoku_read_model_jizoku_test}
    ...
    test "inspect_run/2 can read from the read model" do
      ...
      assert {:ok, %Snapshot{} = snapshot} =
               Jizoku.inspect_run(@read_model_run_id,
                 read_model: :read_model,
    ...
    test "start/3 appends journal start and dispatch facts" do
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = graph} =
               Jizoku.inspect_run_graph(snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_started_at
      ...
      assert Jizoku.Runtime.RunIndexProjection.run_ids(run_index_projection) == [
               snapshot.run_id
             ]
    end
    ...
    test "partitions isolate identical run, queue, index, catalog, and execution identities" do
      ...
      assert {:ok, acme_storage} =
               Jizoku.Runtime.Journal.Storage.scope(@read_model_storage, "tenant_acme")

      assert {:ok, globex_storage} =
               Jizoku.Runtime.Journal.Storage.scope(@read_model_storage, "tenant_globex")
      ...
      assert Jizoku.Runtime.RunIndexProjection.run_ids(acme_index) == [run_id, acme_only_run_id]
      assert Jizoku.Runtime.RunIndexProjection.run_ids(globex_index) == [run_id]
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{partition: "tenant_acme"} = graph} =
               Jizoku.inspect_run_graph(run_id, shared_read_opts)

      assert Jizoku.Runs.GraphInspection.to_map(graph).partition == "tenant_acme"

      assert {:ok, %Jizoku.Runs.DynamicWorkPreview{partition: "tenant_acme"} = dynamic_preview} =
               Jizoku.preview_dynamic_work(
                 run_id,
                 %{
      ...
      assert Jizoku.Runs.DynamicWorkPreview.to_map(dynamic_preview).partition == "tenant_acme"

      assert {:ok, %Jizoku.ReadModel.Timeline{partition: "tenant_acme"}} =
               Jizoku.inspect_run_timeline(run_id, shared_read_opts)

      assert {:ok, %Diagnostic{partition: "tenant_acme"}} =
               Jizoku.explain_run(run_id, shared_read_opts)
    ...
    test "inspect, graph, and explanation expose durable compensation policy" do
      ...
      assert {:ok, %Diagnostic{} = diagnostic} =
               Jizoku.explain_run(snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_started_at
    ...
    test "execute_next/1 recovers a failed saga attempt by planning compensation" do
      ...
      assert {:ok, %Snapshot{reason: :waiting_for_dispatch}} =
               Jizoku.inspect_run(started_snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: DateTime.add(@read_model_visible_at, 5, :second)
    ...
    test "graph and explanation surface compensation before dispatch scheduling recovers" do
      ...
      assert {:ok, graph} =
               Jizoku.inspect_run_graph(started_snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: DateTime.add(@read_model_visible_at, 5, :second)
      ...
      assert {:ok, diagnostic} =
               Jizoku.explain_run(started_snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: DateTime.add(@read_model_visible_at, 5, :second)
    ...
    test "cancel/2 prevents scheduled deferred continuations from running later" do
      ...
      assert {:ok, %Snapshot{} = cancelled_snapshot} =
               Jizoku.cancel(started_snapshot.run_id,
                 runtime: :journal,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
      ...
      executor_prefix = [:jizoku, :runtime, :executor, :execute_next]
      executor_events = Enum.map([:start, :stop], &Enum.concat(executor_prefix, [&1]))
      Jizoku.Test.TelemetryCapture.attach(executor_events)
    ...
    test "replay/2 starts fresh work instead of copying deferred continuation state" do
      ...
      Jizoku.Test.TelemetryCapture.attach(replay_events)

      assert {:ok, %Snapshot{} = replay} =
               Jizoku.replay(source.run_id,
                 runtime: :journal,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
    ...
    test "inspect, graph, and explanation expose recorded dynamic work metadata" do
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = graph} =
               Jizoku.inspect_run_graph(@read_model_run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_visible_at
      ...
      assert {:ok, %Diagnostic{} = diagnostic} =
               Jizoku.explain_run(@read_model_run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_visible_at
    ...
    test "schedule_dynamic_work/3 records dynamic work and dispatches executable nodes" do
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = graph} =
               Jizoku.inspect_run_graph(started.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_visible_at
    ...
    test "preview and record dynamic work validate node actions through an action registry" do
      ...
      assert {:ok, %Jizoku.Runs.DynamicWorkPreview{}} =
               Jizoku.preview_dynamic_work(
                 @read_model_run_id,
                 attrs,
                 read_model: :read_model,
    ...
    test "preview_dynamic_work/3 validates inspectable dynamic work without appending" do
      ...
      assert {:ok, %Jizoku.Runs.DynamicWorkPreview{} = preview} =
               Jizoku.preview_dynamic_work(
                 @read_model_run_id,
                 %{
                   dynamic_key: "subscription_digest_fanout",
    ...
    test "preview_dynamic_work/3 marks exact duplicate dynamic work without appending" do
      ...
      assert %{
               ...
               added_edge_ids: [],
               warnings: [:duplicate_dynamic_work]
             } = Jizoku.Runs.DynamicWorkPreview.to_map(preview)
    ...
    test "record_dynamic_work/3 rejects terminal runs and append conflicts" do
      ...
      assert {:error, {:invalid_dynamic_work, {:run, :terminal}}} =
               Jizoku.record_dynamic_work(@read_model_run_id, attrs,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_visible_at
    ...
    test "record_dynamic_work/3 rejects missing run ids without creating a run thread" do
      ...
      assert {:error, :not_found} =
               Jizoku.inspect_run("missing_run",
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_visible_at
    ...
    test "apply_signal/2 starts journal runs through a durable signal receipt" do
      ...
      Jizoku.Test.TelemetryCapture.attach(lifecycle_events)
    ...
    test "start/3 rejects invalid default-trigger payloads" do
      assert {:error, {:invalid_payload, :expected_map}} =
               Jizoku.start(PaymentRecoveryWorkflow, :invalid_payload,
                 runtime: :journal,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_started_at
    ...
    test "concise convenience arities use configured runtime defaults" do
      ...
      assert {:ok, %Snapshot{} = default_trigger_snapshot} =
               Jizoku.start(PaymentRecoveryWorkflow, %{account_id: "acct_concise_start"})

      assert {:ok, %Snapshot{} = named_trigger_snapshot} =
               Jizoku.start(PaymentRecoveryWorkflow, :gateway_recovery, %{
      ...
      assert {:error, :not_found} = Jizoku.resume(missing_run_id)
      assert {:error, :not_found} = Jizoku.resume(missing_run_id, runtime: :journal)
      assert {:error, :not_found} = Jizoku.resume(missing_run_id, attrs)
      assert {:error, :not_found} = Jizoku.approve(missing_run_id, attrs)
      assert {:error, :not_found} = Jizoku.reject(missing_run_id, attrs)
    end
    ...
    test "concise control functions preserve existing public error shapes" do
      ...
      assert {:error, :not_found} = Jizoku.resume(missing_run_id, attrs, opts)
      assert {:error, :not_found} = Jizoku.approve(missing_run_id, attrs, opts)
      assert {:error, :not_found} = Jizoku.reject(missing_run_id, attrs, opts)
      assert {:error, :not_found} = Jizoku.cancel(missing_run_id, opts)
      assert {:error, :not_found} = Jizoku.replay(missing_run_id, opts)

      assert {:error, :invalid_run_id} = Jizoku.resume(malformed_run_id, attrs, opts)
      assert {:error, :invalid_run_id} = Jizoku.approve(malformed_run_id, attrs, opts)
      assert {:error, :invalid_run_id} = Jizoku.reject(malformed_run_id, attrs, opts)
      assert {:error, :invalid_run_id} = Jizoku.cancel(malformed_run_id, opts)
      assert {:error, :invalid_run_id} = Jizoku.replay(malformed_run_id, opts)
    end

    test "concise API keeps supported runtime names explicit" do
      refute function_exported?(Jizoku, :inspect, 2)
      assert function_exported?(Jizoku, :start, 2)
      assert function_exported?(Jizoku, :start, 3)
      assert function_exported?(Jizoku, :start, 4)
      assert function_exported?(Jizoku, :resume, 1)
      assert function_exported?(Jizoku, :resume, 2)
      assert function_exported?(Jizoku, :resume, 3)
      assert function_exported?(Jizoku, :approve, 2)
      assert function_exported?(Jizoku, :approve, 3)
      assert function_exported?(Jizoku, :reject, 2)
      assert function_exported?(Jizoku, :reject, 3)
      assert function_exported?(Jizoku, :cancel, 1)
      assert function_exported?(Jizoku, :cancel, 2)
      assert function_exported?(Jizoku, :replay, 1)
      assert function_exported?(Jizoku, :replay, 2)
      assert function_exported?(Jizoku, :inspect_run, 2)
      assert function_exported?(Jizoku, :inspect_run_graph, 2)
      assert function_exported?(Jizoku, :explain_run, 2)
    end
    ...
    test "start_child_run/4 starts a deterministic child and links it to the parent" do
      ...
      assert {:ok, %Snapshot{} = inspected_parent} =
               Jizoku.inspect_run(parent.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_visible_at
    ...
    test "child runs inherit the parent partition" do
      ...
      assert {:error, :not_found} =
               Jizoku.inspect_run(child.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
    ...
    test "start_child_run/4 is idempotent for duplicate child keys" do
      ...
      assert {:error, {:invalid_parent_context, :workflow}} =
               Jizoku.start_child_run(
                 %Jizoku.Step.Context{parent_context | workflow: RepoTransactionWorkflow},
                 ChildDigestWorkflow,
                 :deliver_digest,
                 %{subscription_id: "sub_123"},
                 child_opts
    ...
    test "start_child_run/4 rejects missing child keys and terminal parents" do
      ...
      assert {:error, {:invalid_parent_context, :run_id}} =
               Jizoku.start_child_run(
                 %Jizoku.Step.Context{parent_context | run_id: nil},
                 ChildDigestWorkflow,
                 :deliver_digest,
                 %{subscription_id: "sub_123"},
                 child_key: "digest_subscription_1",
      ...
      assert {:error, {:invalid_parent_context, :origin}} =
               Jizoku.start_child_run(
                 %Jizoku.Step.Context{parent_context | runnable_key: nil},
                 ChildDigestWorkflow,
                 :deliver_digest,
                 %{subscription_id: "sub_123"},
                 child_key: "digest_subscription_1",
      ...
      assert {:error, {:invalid_parent_context, :attempt}} =
               Jizoku.start_child_run(
                 %Jizoku.Step.Context{parent_context | attempt: "one"},
                 ChildDigestWorkflow,
                 :deliver_digest,
                 %{subscription_id: "sub_123"},
                 child_key: "digest_subscription_1",
    ...
    test "child starter validates direct journal boundary inputs" do
      parent_context = %Jizoku.Step.Context{
        run_id: Ecto.UUID.generate(),
      ...
      assert {:error, {:invalid_payload, :expected_map}} =
               Jizoku.Runtime.Journal.ChildStarter.start_child_run(
                 parent_context,
                 ChildDigestWorkflow,
                 :deliver_digest,
                 :invalid_payload,
      ...
      assert {:error, {:partition_mismatch, :child}} =
               Jizoku.Runtime.Journal.ChildStarter.start_child_run(
                 %Jizoku.Step.Context{parent_context | partition: "tenant_acme"},
                 ChildDigestWorkflow,
                 :deliver_digest,
                 %{subscription_id: "sub_123"},
    ...
    test "start_child_run/4 repairs a missing parent link for an existing child" do
      ...
      assert {:ok, child_run_id} =
               Jizoku.Runtime.ScheduleIdentity.run_id(
    ...
    test "start_child_run/4 reuses string-keyed persisted parent links" do
      ...
      link_entry = %Jizoku.Runtime.DispatchProtocol.Entry{
      ...
      assert {:error, :conflict} =
               Jizoku.start_child_run(
                 ...
                 journal_storage:
                   {FaultInjectingStorage,
                    delegate: @read_model_storage,
                    conflict_thread_id: "jizoku:run:#{parent.run_id}"},
                 queue: @read_model_queue,
                 now: DateTime.add(@read_model_visible_at, 1, :second)
               )
    end
      ...
      assert {:error, :append_failed} =
               Jizoku.start_child_run(
                 ...
                 journal_storage:
                   {FaultInjectingStorage,
                    delegate: @read_model_storage,
                    fail_append_thread_id: "jizoku:run:#{parent.run_id}"},
                 queue: @read_model_queue,
                 now: DateTime.add(@read_model_visible_at, 1, :second)
               )
    end
    ...
    test "start_child_run/4 rejects malformed existing child links for the same child" do
      ...
      malformed_link = %Jizoku.Runtime.DispatchProtocol.Entry{
        type: :child_run_started,
        thread: {:run, parent.run_id},
    ...
    test "start_child_run/4 ignores malformed origins when checking child key reuse" do
      ...
      malformed_origin_link = %Jizoku.Runtime.DispatchProtocol.Entry{
        type: :child_run_started,
        thread: {:run, parent.run_id},
        data: %{
          run_id: parent.run_id,
    ...
    test "cancel/2 rejects malformed checkpoint child links without raising" do
      ...
      projection =
        thread.entries
        |> Jizoku.Runtime.WorkflowAgent.Projection.rebuild()
        |> Map.put(:child_runs, [
      ...
      assert {:error, :load_failed} =
               Jizoku.cancel(parent.run_id,
                 ...
                 journal_storage:
                   {FaultInjectingStorage,
                    delegate: @read_model_storage,
                    fail_load_thread_id: "jizoku:run:#{child_run_id}"},
                 queue: @read_model_queue,
                 now: DateTime.add(@read_model_visible_at, 1, :second)
               )
    end
    ...
    test "journal starter ignores non-map initial context at the internal boundary" do
      assert {:ok, %Snapshot{} = child} =
               Jizoku.Runtime.Journal.Starter.start_run(
                 ChildDigestWorkflow,
                 :deliver_digest,
    ...
    test "start_child_run/4 keeps child identity stable across parent retry runnable keys" do
      ...
      assert {:ok, %Snapshot{} = inspected_parent} =
               Jizoku.inspect_run(parent_run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue
               )
    ...
    test "start_child_run/4 uses the persisted parent link for retry after linked crash" do
      ...
      retry_context = %Jizoku.Step.Context{
        run_id: parent_run_id,
        workflow: PaymentRecoveryWorkflow,
        step: :check_gateway,
        attempt: 2,
    ...
    test "list_runs/2 lists journal runs for one workflow newest first" do
      ...
      assert {:ok, [%Summary{} = first, %Summary{} = second]} =
               Jizoku.list_runs([workflow: PaymentRecoveryWorkflow],
                 runtime: :journal,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_visible_at
    ...
    test "list_runs/2 uses the queue recorded in each run catalog fact" do
      ...
      assert {:ok, %Snapshot{} = inspected} =
               Jizoku.inspect_run(listed_second.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = graph} =
               Jizoku.inspect_run_graph(listed_second.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: listed_second.queue,
                 now: @read_model_visible_at
    ...
    test "apply_signal/2 applies public Jizoku control signals" do
      cancelled_at = DateTime.add(@read_model_visible_at, 1, :second)
    ...
    test "apply_signal/2 treats nil and explicit default triggers as the same idempotent start" do
      ...
      assert {:ok, %Snapshot{} = started} =
               Jizoku.apply_signal(default_signal,
                 runtime: :journal,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue
               )
      ...
      assert {:ok, %Snapshot{} = duplicate} =
               Jizoku.apply_signal(explicit_signal,
                 runtime: :journal,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue
               )
    ...
    test "apply_signal/2 rejects duplicate start idempotency keys with different payloads" do
      ...
      assert {:error, :conflict} =
               Jizoku.apply_signal(conflicting_signal,
                 runtime: :journal,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue
               )
      ...
      assert {:ok, inspected} =
               Jizoku.inspect_run(started.run_id,
                 runtime: :journal,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
    ...
    test "apply_signal/2 rejects malformed signal application requests" do
      assert {:error, :invalid_signal} = Jizoku.apply_signal(%{})

      assert {:ok, %Signal{} = signal} = Signal.cancel_run(Ecto.UUID.generate())

      assert {:error, {:invalid_option, {:opts, :invalid}}} =
               Jizoku.apply_signal(signal, :bad_opts)
    end
    ...
    test "replay/2 returns structured journal errors for missing and malformed run ids" do
      assert {:error, :not_found} =
               Jizoku.replay(Ecto.UUID.generate(),
                 runtime: :journal,
                 journal_storage: @read_model_storage
               )

      assert {:error, :invalid_run_id} =
               Jizoku.replay("not-a-uuid",
                 runtime: :journal,
                 journal_storage: @read_model_storage
               )
    end
    ...
    test "cancel/2 returns structured journal errors for missing and malformed run ids" do
      assert {:error, :not_found} =
               Jizoku.cancel(Ecto.UUID.generate(),
                 runtime: :journal,
                 journal_storage: @read_model_storage
               )

      assert {:error, :invalid_run_id} =
               Jizoku.cancel("not-a-uuid",
                 runtime: :journal,
                 journal_storage: @read_model_storage
               )
    ...
    test "list_runs/2 surfaces malformed journal run catalog facts" do
      ...
      malformed_entry = %Jizoku.Runtime.DispatchProtocol.Entry{
        type: :run_cataloged,
    ...
    test "list_runs/2 surfaces conflicting journal run catalog facts" do
      ...
      first_entry = %Jizoku.Runtime.DispatchProtocol.Entry{
        type: :run_cataloged,
        thread: {:run_catalog, "all"},
        data: %{
          run_id: run_id,
      ...
      second_entry = %Jizoku.Runtime.DispatchProtocol.Entry{
        type: :run_cataloged,
        thread: {:run_catalog, "all"},
        data: %{
          run_id: run_id,
    ...
    test "configured journal runtime defaults start, inspect, explain, and execute calls" do
      ...
      assert {:ok, %Snapshot{} = inspected} =
               Jizoku.inspect_run(started.run_id, now: @read_model_started_at)

      assert inspected.run_id == started.run_id
      assert inspected.queue == configured_queue

      assert {:ok, %Diagnostic{} = explanation} =
               Jizoku.explain_run(started.run_id, now: @read_model_started_at)
      ...
      assert {:error, {:invalid_option, {:queue, :invalid}}} =
               Jizoku.inspect_run(started.run_id, queue: "../dispatch")

      assert {:error, {:invalid_option, {:queue, :invalid}}} =
               Jizoku.execute_next(queue: "../dispatch")
    ...
    test "journal runtime executes built-in pause steps into durable manual state" do
      ...
      assert {:error, :not_found} = Jizoku.resume(run_id, %{})
    ...
    test "journal runtime returns structured errors for invalid pause resume requests" do
      assert {:error, :not_found} =
               Jizoku.resume(Ecto.UUID.generate(),
                 journal_storage: @read_model_storage
               )

      assert {:error, :not_found} =
               Jizoku.resume(Ecto.UUID.generate(), %{}, journal_storage: @read_model_storage)
    ...
    test "journal runtime repairs pending dispatch after approval resolution was already committed" do
      ...
      assert {:ok, approved_signal_receipt} =
               Jizoku.Runtime.Journal.CommandReceipt.new(
                 :approve_run,
                 %{
                   run_id: run_id,
    ...
    test "journal runtime returns structured errors for invalid approval controls" do
      assert {:error, {:invalid_review, %{actor: :required}}} =
               Jizoku.approve(Ecto.UUID.generate(), %{}, journal_storage: @read_model_storage)
    ...
    test "inspect_run_graph/2 identifies claimed journal attempts as the current node" do
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = graph_with_anomaly} =
               Jizoku.inspect_run_graph(snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
    ...
    test "journal runtime start rejects removed public options" do
      ...
      assert {:error, {:invalid_option, {:journal_storage, String}}} =
               Jizoku.start(
                 ...
                 %{account_id: "acct_123"},
                 runtime: :journal,
                 journal_storage: {String, path: "/tmp/jizoku_storage", token: "redacted"}
               )
    ...
    test "journal runtime start repairs a partially appended run thread" do
      ...
      assert Jizoku.Runtime.RunIndexProjection.run_ids(run_index_projection) == [run_id]
    end
    ...
    test "execute_next/1 exposes safe attempt metadata to native step context" do
      storage = {Jido.Storage.ETS, table: :jizoku_native_attempt_context_test}
    ...
    test "execute_next/1 exposes safe attempt metadata to raw Jido action context" do
      storage = {Jido.Storage.ETS, table: :jizoku_raw_action_attempt_context_test}
    ...
    test "execute_next/1 rejects native continuation inside a repo transaction" do
      ...
      on_exit(fn ->
        case previous_activation do
          {:ok, value} -> Application.put_env(:jizoku, :continuation_fences, value)
          :error -> Application.delete_env(:jizoku, :continuation_fences)
        end
      end)
    ...
    test "repo transaction telemetry flushes after commit and is discarded after completion rollback" do
      ...
      Jizoku.Test.TelemetryCapture.attach(events)
      ...
      executor_stop = [:jizoku, :runtime, :executor, :execute_next, :stop]
      Jizoku.Test.TelemetryCapture.attach([executor_stop])
    ...
    test "executor and step spans enclose committed lifecycle events in order" do
      ...
      executor_prefix = [:jizoku, :runtime, :executor, :execute_next]
      step_prefix = [:jizoku, :runtime, :step, :execute]
      ...
      assert_receive {:telemetry_event, [:jizoku, :runtime, :attempt, :claimed], %{count: 1},
                      claimed_metadata}
    ...
    test "executor exceptions emit sanitized exception spans and preserve raises" do
      ...
      prefix = [:jizoku, :runtime, :executor, :execute_next]
    ...
    test "execute_next/1 schedules retry attempts through the journal dispatch projection" do
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = graph_with_history} =
               Jizoku.inspect_run_graph(started_snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
    ...
    test "graph inspection serializes completed runs with details redacted by default" do
      ...
      history_payload = Jizoku.Runs.GraphInspection.to_map(graph_with_history)
    ...
    test "graph inspection serializes dependency, paused, retrying, and failed states" do
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = dependency_graph} =
               Jizoku.inspect_run_graph(dependency_snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: @read_model_visible_at
      ...
      dependency_payload = Jizoku.Runs.GraphInspection.to_map(dependency_graph)
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = approval_graph} =
               Jizoku.inspect_run_graph(approval_snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: DateTime.add(@read_model_visible_at, 1, :minute),
      ...
      approval_payload = Jizoku.Runs.GraphInspection.to_map(approval_graph)
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = retrying_graph} =
               Jizoku.inspect_run_graph(retry_snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
                 now: DateTime.add(@read_model_visible_at, 2, :minute)
      ...
      retrying_payload = Jizoku.Runs.GraphInspection.to_map(retrying_graph)
      ...
      assert {:ok, %Jizoku.Runs.GraphInspection{} = failed_graph} =
               Jizoku.inspect_run_graph(retry_snapshot.run_id,
                 read_model: :read_model,
                 journal_storage: @read_model_storage,
                 queue: @read_model_queue,
      ...
      failed_payload = Jizoku.Runs.GraphInspection.to_map(failed_graph)
    ...
    test "execute_next/1 durably fails native steps that raise arbitrary exceptions" do
      ...
      Jizoku.Test.TelemetryCapture.attach(span_events)
    ...
    test "read model infers Ecto journal storage from the configured repo" do
      assert {:error, :not_found} =
               Jizoku.inspect_run(@read_model_run_id, read_model: :read_model)

      assert {:error, :not_found} =
               Jizoku.explain_run(@read_model_run_id, read_model: :read_model)
    end
    ...
    test "returns a structured error for unsupported read models" do
      assert {:error, {:invalid_option, {:read_model, :invalid}}} =
               Jizoku.inspect_run(@read_model_run_id, read_model: :unknown)

      assert {:error, {:invalid_option, {:read_model, :invalid}}} =
               Jizoku.explain_run(@read_model_run_id, read_model: :unknown)

      assert {:error, {:invalid_option, {:read_model, :invalid}}} =
               Jizoku.inspect_run(@read_model_run_id, read_model: :unsupported)

      assert {:error, {:invalid_option, {:read_model, :invalid}}} =
               Jizoku.explain_run(@read_model_run_id, read_model: :unsupported)
    end
    ...
    test "returns a structured error for malformed option lists" do
      assert {:error, {:invalid_option, {:opts, :invalid}}} =
               Jizoku.inspect_run(@read_model_run_id, [:bad])

      assert {:error, {:invalid_option, {:opts, :invalid}}} =
               Jizoku.explain_run(@read_model_run_id, [:bad])
    end
    ...
    test "read model rejects malformed run ids without raising" do
      assert {:error, {:invalid_option, {:run_id, :invalid}}} =
               Jizoku.inspect_run(123,
                 read_model: :read_model,
                 journal_storage: @read_model_storage
               )

      assert {:error, {:invalid_option, {:run_id, :invalid}}} =
               Jizoku.explain_run(123,
                 read_model: :read_model,
                 journal_storage: @read_model_storage
               )
    end
    ...
    test "read model redacts invalid option values" do
      ...
      assert {:error, reason} =
               Jizoku.explain_run(secret_value,
                 read_model: :read_model,
                 journal_storage: @read_model_storage
               )
  ...
  defp warm_read_model_storage do
    ...
    assert :ok =
             Journal.put_checkpoint(
               ...
               Jizoku.Runtime.RunIndexProjection.new("StorageSeedWorkflow"),
               1
             )
  end
  ...
  defp read_model_table_name(:checkpoints),
    do: :jizoku_read_model_jizoku_test_checkpoints

  defp read_model_table_name(:threads),
    do: :jizoku_read_model_jizoku_test_threads

  defp read_model_table_name(:thread_meta),
    do: :jizoku_read_model_jizoku_test_thread_meta
  ...
  defp put_jizoku_config(overrides) do
